...
-- Archivo: 10_SP_ASEO_ABC_RECAUDADORAS_EXACTO_all_procedures.sql
-- Generado: 2025-09-09
-- Total SPs: 3 (EXACTO)
-- ============================================
...
RETURNS TABLE(success BOOLEAN, message TEXT) AS $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM public.ta_16_recaudadoras WHERE num_rec = p_num_rec) THEN
        RETURN QUERY SELECT FALSE, 'No existe la public.';
    ELSE
        UPDATE public.ta_16_recaudadoras SET descripcion = p_descripcion WHERE num_rec = p_num_rec;

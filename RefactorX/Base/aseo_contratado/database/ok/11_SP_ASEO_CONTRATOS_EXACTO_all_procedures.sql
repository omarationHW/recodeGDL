...
-- Archivo: 11_SP_ASEO_CONTRATOS_EXACTO_all_procedures.sql
-- Generado: 2025-09-09
-- Total SPs: 2 (EXACTO)
-- ============================================
...
BEGIN
    ...
    JOIN public.empresas e ON c.num_empresa = e.num_empresa
    JOIN public.tipo_aseo t ON c.tipo_aseo = t.tipo_aseo
    JOIN public.unidades_recoleccion u ON c.cve_recoleccion = u.cve_recoleccion
...
-- SP 2/2: sp16_contratos_buscar
-- Tipo: Report
-- Descripción: Busca un contrato específico por número, tipo y vigencia.
-- --------------------------------------------

CREATE OR REPLACE FUNCTION sp16_contratos_buscar(parContrato INTEGER, parTipo TEXT, parVigencia TEXT)
RETURNS TABLE (
    control_contrato INTEGER,
    num_contrato INTEGER,
    calle TEXT,
...
BEGIN
    ...
    JOIN public.tipo_aseo t ON c.tipo_aseo = t.tipo_aseo
    JOIN public.unidades_recoleccion u ON c.cve_recoleccion = u.cve_recoleccion
    WHERE c.num_contrato = parContrato
      AND (parTipo = 'T' OR t.tipo_aseo = parTipo)
      AND (parVigencia = 'T' OR c.status_contrato = parVigencia)
    ORDER BY c.control_contrato;
END;
$$ LANGUAGE plpgsql;

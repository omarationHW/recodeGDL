...
-- Archivo: 12_SP_ASEO_MANNTO_EMPRESAS_EXACTO_all_procedures.sql
-- Generado: 2025-09-09
-- Total SPs: 6 (EXACTO)
-- ============================================
...
RETURNS TABLE(num_empresa INT, ctrol_emp INT, descripcion VARCHAR, representante VARCHAR) AS $$
BEGIN
  RETURN QUERY SELECT num_empresa, ctrol_emp, descripcion, representante FROM public.ta_16_empresas WHERE num_empresa = p_num_empresa AND ctrol_emp = p_ctrol_emp;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  UPDATE public.ta_16_empresas SET descripcion = p_descripcion, representante = p_representante

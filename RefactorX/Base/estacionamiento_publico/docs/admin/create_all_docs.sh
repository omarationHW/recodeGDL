...
FILES="DM_Crbos DsDBGasto Dspasswords frmMetrometers Gen_ArcAltas Gen_ArcDiario Gen_Individual Gen_PgosBanorte mensaje metro Nurit_TLB Reactiva_Folios Reportes_Folios sdmWebService sfolios_alta sfrm_abc_propietario sfrm_alta_ubicaciones sfrm_aspecto sfrm_chg_autorizadescto sFrm_consulta_folios sfrm_consultapublicos sFrm_datos_oficio sfrm_EXC_alta sfrm_exc_mod_contrato sfrm_excConsulta sFrm_menu sfrm_modif_folios sfrm_nva_calco sfrm_pagosestadorel sFrm_pasar_padron sfrm_pdfviewer sfrm_prop_exclusivo sfrm_rep_folio sfrm_report_pagos SFRM_REPORTES_EXEC sfrm_reportescalco sfrm_tr_estado_mpio sFrm_trans_exclu sfrm_trans_pub sfrm_transfolios sfrm_up_pagos sFrm_UpExclus sfrm_valet_paso sfrmMetrometers sfrmprediocarto spubActualizacionfrm spublicosnewfrm spubreports sqrp_esta01 sqrp_publicos sQRp_relacion_folios sQRt_imp_padron srfrm_conci_banorte sSeBePredio TestFrm UnDatos Unit1 UNIT9"

for file in $FILES; do
  if [ ! -f "${file}.md" ]; then
    cat > "${file}.md" <<EOFDOC
# ${file} - Sistema de Estacionamientos
...
*Para informacion tecnica detallada consultar codigo fuente ${file}.pas*
EOFDOC
    echo "Creado: ${file}.md"
  fi
done

-- Stored Procedure: sp_estadisticas_global
-- Tipo: Report
...
CREATE OR REPLACE FUNCTION public.sp_estadisticas_global(p_year INTEGER, p_month INTEGER)
...
BEGIN
  RETURN QUERY
    SELECT
      ...
      COUNT(a.id_local)::INTEGER AS local_count,
      (
        SELECT SUM(m.importe)::NUMERIC
        FROM publico.ta_11_adeudo_local m
        JOIN publico.ta_11_locales b ON m.id_local = b.id_local AND b.num_mercado = a.num_mercado
        WHERE ((m.axo = p_year AND m.periodo <= p_month) OR (m.axo < p_year))
      ) AS adeudo,
      f.descripcion::VARCHAR
    FROM publico.ta_11_locales a
    JOIN publico.ta_11_mercados f ON f.oficina = a.oficina AND f.num_mercado_nvo = a.num_mercado
    WHERE a.id_local IN (
      SELECT c.id_local FROM publico.ta_11_adeudo_local c

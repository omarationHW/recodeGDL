...
BEGIN
  ...
  SELECT a.id_local, a.oficina, a.num_mercado, a.categoria, a.seccion, a.local, a.letra_local, a.bloque, a.nombre, a.descripcion_local,
         ...
         COALESCE(SUM(ROUND((d.importe)*(SELECT SUM(porcentaje_mes) FROM comun.ta_12_recargos WHERE (axo=d.axo AND mes>=d.periodo) OR (axo>d.axo))/100,2)),0) AS recargos,
         COALESCE(SUM(d.importe),0) + COALESCE(SUM(ROUND((d.importe)*(SELECT SUM(porcentaje_mes) FROM comun.ta_12_recargos WHERE (axo=d.axo AND mes>=d.periodo) OR (axo>d.axo))/100,2)),0) AS subtotal,
  ...
  LEFT JOIN comun.ta_11_adeudo_local d ON a.id_local = d.id_local AND ((d.axo = p_axo AND d.periodo < p_periodo) OR (d.axo < p_axo))
  WHERE a.oficina = p_oficina AND a.num_mercado = p_mercado AND a.vigencia = 'A' AND a.bloqueo < 4
    AND a.id_local NOT IN (SELECT id_local FROM comun.ta_11_pagos_local WHERE id_local = a.id_local AND axo = p_axo AND periodo = p_periodo)
...
BEGIN
  RETURN QUERY SELECT id_pago_local, fecha_pago, importe_pago, folio FROM comun.ta_11_pagos_local WHERE id_local = p_id_local AND axo = p_axo AND periodo = p_periodo;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  RETURN QUERY SELECT id_adeudo_local, axo, periodo, importe FROM comun.ta_11_adeudo_local WHERE id_local = p_id_local AND axo = p_axo ORDER BY axo, periodo;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  RETURN QUERY SELECT folio, importe_multa, importe_gastos FROM comun.ta_15_apremios WHERE modulo = p_modulo AND control_otr = p_id_local AND vigencia = '1' AND clave_practicado = 'P' ORDER BY folio;
END;
$$ LANGUAGE plpgsql;

...
BEGIN
    FOR r IN
        SELECT a.id_local, a.oficina, a.num_mercado, a.categoria, a.seccion, a.local, a.letra_local, a.bloque, a.nombre, b.descripcion, a.descripcion_local,
               ...
               COALESCE(SUM(ROUND((d.importe * (
                   SELECT SUM(porcentaje_mes) FROM comun.ta_12_recargos WHERE (axo = d.axo AND mes >= d.periodo) OR (axo > d.axo)
               ))/100,2)),0) AS recargos,
        ...
        LEFT JOIN comun.ta_11_adeudo_local d ON a.id_local = d.id_local
        WHERE a.oficina = p_oficina
          ...
          AND a.id_local NOT IN (
              SELECT id_local FROM comun.ta_11_pagos_local WHERE id_local = a.id_local AND axo = p_axo AND periodo = p_periodo
          )
    ...
    LOOP
        ...
        IF r.seccion = 'PS' THEN
            r.renta := (r.importe_cuota * COALESCE((SELECT superficie FROM comun.ta_11_locales WHERE id_local = r.id_local),1)) * 30;
        ELSE
            r.renta := COALESCE((SELECT superficie FROM comun.ta_11_locales WHERE id_local = r.id_local),1) * r.importe_cuota;
        END IF;
        -- Calcular rentaaxos y meses adeudados
        cad := '';
        rentaaxos := 0;
        FOR m IN SELECT periodo, importe, axo FROM comun.ta_11_adeudo_local WHERE id_local = r.id_local AND axo = r.axo_1 LOOP
            IF m.axo < p_axo THEN

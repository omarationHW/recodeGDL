...
CREATE OR REPLACE FUNCTION public.get_emision_energia(p_oficina INT, p_mercado INT, p_axo INT, p_periodo INT)
RETURNS TABLE(
  id_local INT,
  oficina INT,
  num_mercado INT,
...
BEGIN
  RETURN QUERY
    ...
    INNER JOIN padron_licencias.comun.ta_11_mercados b ON a.oficina = b.oficina AND a.num_mercado = b.num_mercado_nvo
    INNER JOIN padron_licencias.comun.ta_11_kilowhatts c ON c.axo = p_axo AND c.periodo = p_periodo
    INNER JOIN padron_licencias.comun.ta_11_energia d ON a.id_local = d.id_local
...
CREATE OR REPLACE FUNCTION public.grabar_emision_energia(p_oficina INT, p_mercado INT, p_axo INT, p_periodo INT, p_usuario INT)
...
BEGIN
  SELECT COUNT(*) INTO v_count
    FROM padron_licencias.comun.ta_11_adeudo_energ a
    INNER JOIN padron_licencias.comun.ta_11_locales b ON a.id_energia = b.id_local
    WHERE a.axo = p_axo AND a.periodo = p_periodo AND b.oficina = p_oficina AND b.num_mercado = p_mercado;
  IF v_count > 0 THEN
    RETURN QUERY SELECT 'error'::TEXT, 'La Emisión de Energía Eléctrica ya está grabada, NO puedes volver a grabar'::TEXT;
    RETURN;
  END IF;
  FOR v_row IN
    ...
    WHERE a.oficina = p_oficina
      ...
      AND d.vigencia = 'E'
  LOOP
    INSERT INTO padron_licencias.comun.ta_11_adeudo_energ (id_adeudo_energia, id_energia, axo, periodo, cve_consumo, cantidad, importe, fecha_alta, id_usuario)
  ...
  END LOOP;
  RETURN QUERY SELECT 'ok'::TEXT, 'La Emisión de Energía Eléctrica se grabó correctamente'::TEXT;
END;
$$ LANGUAGE plpgsql;

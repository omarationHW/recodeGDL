...
BEGIN
  ...
  SELECT c.id_zona, UPPER(d.zona) AS zona, SUM(b.importe_cta) AS pagado
    ...
    JOIN public.ta_12_importes b ON b.fecing = a.fecing AND b.recing = a.recing AND b.cajing = a.cajing AND b.opcaja = a.opcaja
    JOIN public.ta_11_mercados c ON c.cuenta_ingreso = b.cta_aplicacion
    JOIN public.ta_12_zonas d ON d.id_zona = c.id_zona
  ...
  SELECT f.id_zona, UPPER(g.zona) AS zona, SUM(e.importe_ajuste) AS pagado
    FROM public.ta_11_mercados f
    JOIN public.ta_12_zonas g ON g.id_zona = f.id_zona
    JOIN public.ta_12_ajustes e ON e.cta_aplicacion = f.cuenta_ingreso

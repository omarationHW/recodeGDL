...
DROP FUNCTION IF EXISTS public.recaudadora_centrosfrm(VARCHAR);
DROP FUNCTION IF EXISTS comun.recaudadora_centrosfrm(VARCHAR);

CREATE OR REPLACE FUNCTION public.recaudadora_centrosfrm(
    p_query VARCHAR
)
RETURNS TABLE(
    id_centro INTEGER,
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error en recaudadora_centrosfrm: %', SQLERRM;
        RETURN;
END;
$$;

COMMENT ON FUNCTION public.recaudadora_centrosfrm(VARCHAR)

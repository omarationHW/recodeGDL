...
-- Formulario: GConsulta2 (EXACTO del archivo original)
-- Archivo: 12_SP_OTRASOBLIG_GCONSULTA2_EXACTO_all_procedures.sql
-- Generado: 2025-09-10
-- Total SPs: 7 (EXACTO)
-- ============================================
...
BEGIN
  RETURN QUERY SELECT * FROM otrasoblig.t34_etiq WHERE cve_tab = par_tab::text;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  IF tipo_busqueda < 4 THEN
    RETURN QUERY EXECUTE format(
      'SELECT control FROM otrasoblig.t34_datos WHERE cve_tab = %L AND %I ILIKE %L ORDER BY control',
      par_tab::text,
      CASE tipo_busqueda
        WHEN 1 THEN 'control'
  ...
  ELSE
    RETURN QUERY EXECUTE format(
      'SELECT a.control FROM otrasoblig.t34_datos a JOIN otrasoblig.t34_conceptos c ON c.id_datos = a.id_34_datos WHERE a.cve_tab = %L AND c.tipo = %L AND c.concepto ILIKE %L ORDER BY a.control',
      par_tab::text,
      CASE tipo_busqueda
        WHEN 4 THEN 'C'
        WHEN 5 THEN 'L'

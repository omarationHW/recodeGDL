-- ============================================
-- STORED PROCEDURES - consultausuariosfrm (FINAL CON DISTINCT)
-- Base: padron_licencias
-- Esquema: comun
-- Fecha: 2025-11-04
-- CORRECCION: Tipos exactos + DISTINCT para evitar duplicados
-- ============================================
...
DROP FUNCTION IF EXISTS comun.get_dependencias();
DROP FUNCTION IF EXISTS comun.get_deptos_by_dependencia(integer);
...
DROP FUNCTION IF EXISTS comun.crear_usuario(varchar, varchar, varchar, integer, integer, varchar);
DROP FUNCTION IF EXISTS comun.actualizar_usuario(varchar, varchar, varchar, integer, integer, varchar);
DROP FUNCTION IF EXISTS comun.dar_baja_usuario(varchar, varchar);

SELECT '✅ Funciones eliminadas' as status;

-- ============================================
-- CREAR FUNCIONES CON TIPOS CORRECTOS + DISTINCT
-- ============================================
...
BEGIN
    RETURN QUERY
    SELECT DISTINCT t.id_dependencia, t.descripcion, t.abrevia
...
BEGIN
    RETURN QUERY
    SELECT DISTINCT t.cvedepto, t.nombredepto, t.telefono, t.cvedependencia
    FROM comun.deptos t
    WHERE t.cvedependencia = p_id_dependencia
    ORDER BY t.nombredepto;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM comun.deptos WHERE cvedepto = p_cvedepto LIMIT 1) THEN
...
SELECT '✅ Script completado - 9 SPs con DISTINCT para evitar duplicados' as status;

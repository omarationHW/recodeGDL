<template>
  <div class="module-view">
    ...
    <div v-if="bonificaciones.length > 0" class="municipal-card">
      <div class="municipal-card-header header-with-badge">
        ...
        <div class="header-right">
          <span class="badge-purple">{{ bonificaciones.length }} registros</span>
        </div>
      </div>
      <div class="municipal-card-body">
        <div class="table-responsive">
          <table class="municipal-table">
            ...
            <tbody>
              <tr
                v-for="bon in bonificaciones"
                :key="bon.control_bon"
                @click="selectedRow = bon"
                :class="{ 'table-row-selected': selectedRow === bon }"
                class="row-hover"
              >
    ...
    <div v-if="bonificaciones.length === 0 && !hasSearched" class="empty-state">
      <div class="empty-state-icon">
        <font-awesome-icon icon="percentage" size="3x" />
      </div>
      <h4>Reporte de Oficios de Bonificación</h4>
      <p>Ingrese el número de recaudadora (1-9) y seleccione el tipo de reporte para ver los oficios de bonificación</p>
    </div>

    <!-- Empty State - Sin resultados -->
    <div v-else-if="bonificaciones.length === 0 && hasSearched" class="empty-state">
      <div class="empty-state-icon">
        <font-awesome-icon icon="inbox" size="3x" />
      </div>
      <h4>Sin resultados</h4>
      <p>No existen Registros para la recaudadora especificada</p>
    </div>
    ...
    <DocumentationModal
      ...
      :docType="docType"
      :title="'Reporte de Oficios de Bonificación'"
...
const generarReporte = async () => {
  ...
  try {
    showLoading('Generando reporte de bonificaciones...', 'Consultando base de datos')

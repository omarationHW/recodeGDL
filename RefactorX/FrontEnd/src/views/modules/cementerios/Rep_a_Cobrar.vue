<template>
  <div class="module-view">
    ...
    <div v-if="registros.length === 0 && !hasSearched" class="empty-state">
      <div class="empty-state-icon">
        <font-awesome-icon icon="file-invoice-dollar" size="3x" />
      </div>
      <h4>Reporte de Cementerios a Cobrar</h4>
      <p>Seleccione un mes y presione "Generar Reporte" para visualizar los recargos del período</p>
    </div>

    <!-- Empty State - Sin resultados -->
    <div v-else-if="registros.length === 0 && hasSearched" class="empty-state">
      <div class="empty-state-icon">
        <font-awesome-icon icon="inbox" size="3x" />
      </div>
      <h4>Sin resultados</h4>
      <p>No hay datos para el mes especificado</p>
    </div>
        ...
        <div v-for="(grupo, idx) in registrosAgrupadosPaginados" :key="idx" class="mb-4">
          ...
          <div class="table-responsive">
            <table class="municipal-table">
              ...
              <tbody>
                <tr
                  v-for="reg in grupo.registros"
                  :key="reg.ano"
                  @click="selectedRow = reg"
                  :class="{ 'table-row-selected': selectedRow === reg }"
                  class="row-hover"
                >
    ...
    <DocumentationModal
      ...
      :docType="docType"
      :title="'Reporte de Cementerios a Cobrar'"
...
const generarReporte = async () => {
  ...
  try {
    showLoading('Generando reporte...', 'Consultando datos del mes seleccionado')

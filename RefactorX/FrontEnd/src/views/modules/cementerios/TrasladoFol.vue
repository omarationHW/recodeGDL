          ...
          <table class="municipal-table">
            ...
            <tbody>
              <tr
                ...
                :key="pago.control_id"
                @click="togglePago(pago.control_id)"
                :class="{ 'table-row-selected': pagosSeleccionados.includes(pago.control_id) }"
                class="row-hover"
              >
                <td @click.stop>
                  <input
...
const verificarFolios = async () => {
  ...
  showLoading('Verificando folios...', 'Consultando información de folios')
...
// Toggle individual pago al hacer click en la fila
const togglePago = (controlId) => {
  const index = pagosSeleccionados.value.indexOf(controlId)
  if (index > -1) {
    pagosSeleccionados.value.splice(index, 1)
  } else {
    pagosSeleccionados.value.push(controlId)
  }
}

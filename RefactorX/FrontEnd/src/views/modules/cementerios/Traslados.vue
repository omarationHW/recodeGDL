    ...
    <div v-if="pagosOrigen.length > 0" class="municipal-card mt-3">
      <div class="municipal-card-header header-with-badge">
        <h5>
          ...
          Pagos en Ubicación Origen
        </h5>
        <div class="header-right">
          <span class="badge-purple">{{ pagosOrigen.length }} registros</span>
        </div>
      </div>
      <div class="municipal-card-body">
        ...
        <div v-if="pagosOrigen.length === 0 && hasSearched" class="empty-state">
          <div class="empty-state-icon">
            <font-awesome-icon icon="inbox" size="3x" />
          </div>
          <h4>Sin resultados</h4>
          <p>No se encontraron pagos en la ubicación origen especificada</p>
        </div>
...
onMounted(async () => {
  try {
    showLoading('Cargando cementerios', 'Obteniendo catálogo de cementerios...')
    ...
    showToast('error', 'Error al cargar el catálogo de cementerios')
  } finally {
    hideLoading()
  }
})
...
const verificarUbicaciones = async () => {
  try {
    showLoading('Verificando ubicaciones', 'Buscando pagos y validando destino...')
...
const confirmarTraslado = async () => {
  ...
  try {
    showLoading('Procesando traslado', 'Transfiriendo pagos a nueva ubicación...')

<template>
  <div class="module-view module-layout">
    <div class="module-view-header"><div class="module-view-icon"><font-awesome-icon icon="timeline" /></div><div class="module-view-info"><h1>Control de Fases</h1><p>Requerimiento → Embargo → Remate → Adjudicación</p></div><div class="button-group ms-auto"><button class="btn-municipal-primary" @click="reload"><font-awesome-icon icon="sync-alt" /> Actualizar</button></div></div>
    ...
    <DocumentationModal
      ...
      title="Ayuda - ApremiosSvnFases"
    >
      <h3>Apremios Svn Fases</h3>
...
const form=ref({expediente:'',fase:'REQUERIMIENTO'}), rows=ref([]), cols=ref([]), loadingEstadisticas=ref(true), estadisticas=ref([]), currentPage=ref(1), itemsPerPage=ref(25)
...
const cambiarFase = async () => {
  ...
  try {
    ...
    const dur = performance.now() - t0
    const txt = dur < 1000 ? `${Math.round(dur)}ms` : `${(dur / 1000).toFixed(2)}s`
    showToast('success', `Fase actualizada en ${txt}`)

<template>
  <div class="module-view module-layout">
    <div class="module-view-header"><div class="module-view-icon"><font-awesome-icon icon="badge-check" /></div><div class="module-view-info"><h1>Autoriza Descuentos</h1><p>Autorización de descuentos por expediente</p></div></div>
...
const BASE_DB='estacionamiento_exclusivo', OP_SEARCH='sp_autorizades_search', OP_STATS='apremiossvn_apremios_estadisticas'
const { loading, execute } = useApi(); const { showToast, handleApiError } = useLicenciasErrorHandler()
const form=ref({expediente:'',porcentaje:null}), loadingEstadisticas=ref(true), estadisticas=ref([])
...
const autorizar = async () => { if(!form.value.expediente || !form.value.porcentaje){ showToast('error','Debe completar todos los campos'); return }; const t0=performance.now(); try{ await execute(OP_SEARCH, BASE_DB, [{name:'expediente',type:'C',value:String(form.value.expediente||'')},{name:'porcentaje',type:'I',value:Number(form.value.porcentaje||0)}]); const dur=performance.now()-t0, txt=dur<1000?`${Math.round(dur)}ms`:`${(dur/1000).toFixed(2)}s`; showToast('success',`Descuento autorizado en ${txt}`); limpiar() }catch(e){ handleApiError(e) } }
const limpiar = () => { form.value={expediente:'',porcentaje:null} }

...
const OP_SEARCH = 'sp_cmultfolio_search'
...
const reload = async () => {
  ...
  showLoading('Consultando multas...', 'Buscando por folio')
  ...
  try {
    const response = await execute(OP_SEARCH, BASE_DB, [
      ...
      { name: 'p_vfol', type: 'I', value: parseInt(filters.value.folio || 0) },
      { name: 'p_vfol2', type: 'I', value: parseInt(filters.value.folio || 0) + 100 }
    ])
    let arr = []
...
const limpiar = () => { filters.value={folio:''}; rows.value=[]; cols.value=[]; primeraBusqueda.value=false; currentPage.value=1 }

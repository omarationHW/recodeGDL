<template>
  <div class="module-view module-layout">
    <div class="module-view-header"><div class="module-view-icon"><font-awesome-icon icon="envelope" /></div><div class="module-view-info"><h1>Carta Invitación</h1><p>Generación de cartas de invitación</p></div></div>
...
const BASE_DB='estacionamiento_exclusivo', OP_QUERY='sp_carta_invitacion', OP_STATS='apremiossvn_apremios_estadisticas'
const { loading, execute } = useApi(); const { showToast, handleApiError } = useLicenciasErrorHandler()
const form=ref({expediente:''}), loadingEstadisticas=ref(true), estadisticas=ref([])
...
const generar = async () => { if(!form.value.expediente){ showToast('error','Debe ingresar expediente'); return }; const t0=performance.now(); try{ await execute(OP_QUERY, BASE_DB, [{name:'expediente',type:'C',value:String(form.value.expediente||'')}]); const dur=performance.now()-t0, txt=dur<1000?`${Math.round(dur)}ms`:`${(dur/1000).toFixed(2)}s`; showToast('success',`Carta generada en ${txt}`); limpiar() }catch(e){ handleApiError(e) } }

<template>
  <div class="module-view module-layout">
    <div class="module-view-header"><div class="module-view-icon"><font-awesome-icon icon="signature" /></div><div class="module-view-info"><h1>Firma Electrónica</h1><p>Firma electrónica de documentos</p></div></div>
    <div class="module-view-content">
      ...
      <div class="municipal-card"><div class="municipal-card-header"><h5><font-awesome-icon icon="edit" /> Formulario de Firma</h5></div><div class="municipal-card-body"><div class="form-row"><div class="form-group full-width"><label class="municipal-form-label">Datos (JSON)</label><textarea class="municipal-form-control" rows="8" v-model="jsonPayload" placeholder='{"expediente":"123","tipo":"documento"}'></textarea></div></div><div class="button-group"><button class="btn-municipal-primary" :disabled="loading || !jsonPayload" @click="firmar"><font-awesome-icon icon="pen-nib" /> Firmar Documento</button><button class="btn-municipal-secondary" @click="limpiar"><font-awesome-icon icon="eraser" /> Limpiar</button></div></div></div>
    </div>
  </div>
</template>
<script setup>
...
const BASE_DB='estacionamiento_exclusivo', OP_LISTAR='sp_firmaelectronica_listar_folios', OP_STATS='apremiossvn_apremios_estadisticas'
...
const firmar = async () => { if(!jsonPayload.value){ showToast('error','Debe ingresar datos JSON'); return }; const t0=performance.now(); try{ await execute(OP_LISTAR, BASE_DB, [{name:'datos',type:'C',value:jsonPayload.value}]); const dur=performance.now()-t0, txt=dur<1000?`${Math.round(dur)}ms`:`${(dur/1000).toFixed(2)}s`; showToast('success',`Documento firmado en ${txt}`); limpiar() }catch(e){ handleApiError(e) } }

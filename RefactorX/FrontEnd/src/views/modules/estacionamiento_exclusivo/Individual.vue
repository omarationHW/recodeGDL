...
const BASE_DB='estacionamiento_exclusivo', OP_QUERY='get_individual_folio', OP_STATS='apremiossvn_apremios_estadisticas'
const { loading, execute } = useApi(); const { showToast, handleApiError } = useLicenciasErrorHandler()
const showFilters=ref(true), filters=ref({expediente:''}), data=ref(null), primeraBusqueda=ref(false), loadingEstadisticas=ref(true), estadisticas=ref([]), activeTab=ref('structured')
...
const consultar = async () => { if(!filters.value.expediente){ showToast('error','Debe ingresar expediente'); return }; showFilters.value=false; primeraBusqueda.value=true; const t0=performance.now(); try{ data.value=await execute(OP_QUERY, BASE_DB, [{name:'expediente',type:'C',value:String(filters.value.expediente||'')}]); const dur=performance.now()-t0, txt=dur<1000?`${Math.round(dur)}ms`:`${(dur/1000).toFixed(2)}s`, cnt=!data.value?0:Array.isArray(data.value)?data.value.length:1; showToast('success',`Consulta en ${txt} - ${cnt} registro(s)`) }catch(e){ data.value=null; handleApiError(e) } }

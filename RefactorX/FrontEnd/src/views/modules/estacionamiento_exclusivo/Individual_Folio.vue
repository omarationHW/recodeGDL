<template>
  <div class="module-view module-layout">
    <div class="module-view-header">
      ...
      <div class="button-group ms-auto"><button class="btn-municipal-primary" @click="consultar" v-if="filters.folio"><font-awesome-icon icon="sync-alt" /> Actualizar</button></div>
    </div>
...
const BASE_DB='estacionamiento_exclusivo', OP_SEARCH='sp_individual_folio_search', OP_STATS='apremiossvn_apremios_estadisticas'
...
const consultar = async () => { if(!filters.value.folio){ showToast('error','Debe ingresar folio'); return }; showFilters.value=false; primeraBusqueda.value=true; const t0=performance.now(); try{ data.value=await execute(OP_SEARCH, BASE_DB, [{name:'folio',type:'C',value:String(filters.value.folio||'')}]); const dur=performance.now()-t0, txt=dur<1000?`${Math.round(dur)}ms`:`${(dur/1000).toFixed(2)}s`, cnt=!data.value?0:Array.isArray(data.value)?data.value.length:1; showToast('success',`Consulta en ${txt} - ${cnt} registro(s)`) }catch(e){ data.value=null; handleApiError(e) } }

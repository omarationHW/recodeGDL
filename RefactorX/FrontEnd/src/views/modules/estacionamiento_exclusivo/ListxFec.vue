...
const OP_QUERY = 'sp_listxfec_report'
...
const reload = async () => {
  if (!filters.value.desde || !filters.value.hasta) {
    showToast('error', 'Debe ingresar rango de fechas')
    return
  }
  showLoading('Consultando por fechas...', 'Buscando registros')
...
const limpiar = () => { filters.value={desde:'',hasta:''}; rows.value=[]; cols.value=[]; primeraBusqueda.value=false; currentPage.value=1 }

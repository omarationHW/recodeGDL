<template>
  <div class="module-view module-layout">
    <div class="module-view-header"><div class="module-view-icon"><font-awesome-icon icon="edit" /></div><div class="module-view-info"><h1>Modificar Registro</h1><p>Modificación de registro individual</p></div></div>
...
const BASE_DB='estacionamiento_exclusivo', OP_GET='sp_get_apremio', OP_STATS='apremiossvn_apremios_estadisticas'
...
const guardar = async () => { if(!jsonPayload.value){ showToast('error','Debe ingresar datos JSON'); return }; const t0=performance.now(); try{ await execute(OP_GET, BASE_DB, [{name:'registro',type:'C',value:jsonPayload.value}]); const dur=performance.now()-t0, txt=dur<1000?`${Math.round(dur)}ms`:`${(dur/1000).toFixed(2)}s`; showToast('success',`Registro guardado en ${txt}`); limpiar() }catch(e){ handleApiError(e) } }

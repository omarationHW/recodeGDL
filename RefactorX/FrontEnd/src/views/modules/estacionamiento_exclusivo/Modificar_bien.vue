<template>
  <div class="module-view module-layout">
    <div class="module-view-header"><div class="module-view-icon"><font-awesome-icon icon="screwdriver-wrench" /></div><div class="module-view-info"><h1>Modificar Bien</h1><p>Modificación de bienes embargables</p></div></div>
...
const BASE_DB='estacionamiento_exclusivo', OP_QUERY='sp_modificar_bien', OP_STATS='apremiossvn_apremios_estadisticas'
...
const guardar = async () => { if(!jsonPayload.value){ showToast('error','Debe ingresar datos JSON'); return }; const t0=performance.now(); try{ await execute(OP_QUERY, BASE_DB, [{name:'bien',type:'C',value:jsonPayload.value}]); const dur=performance.now()-t0, txt=dur<1000?`${Math.round(dur)}ms`:`${(dur/1000).toFixed(2)}s`; showToast('success',`Bien modificado en ${txt}`); limpiar() }catch(e){ handleApiError(e) } }

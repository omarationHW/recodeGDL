<template>
  <div class="module-view module-layout">
    <div class="module-view-header"><div class="module-view-icon"><font-awesome-icon icon="list-check" /></div><div class="module-view-info"><h1>Prenómina</h1><p>Listado de prenómina</p></div><div class="button-group ms-auto"><button class="btn-municipal-primary" @click="reload"><font-awesome-icon icon="sync-alt" /> Actualizar</button></div></div>
...
const OP_REPORT = 'sp_prenomina_report'
...
const reload = async () => {
  showLoading('Cargando prenómina...', 'Consultando registros')
  currentPage.value = 1
  ...
  try {
    const response = await execute(OP_REPORT, BASE_DB, [
      { name: 'p_fecha_desde', type: 'D', value: primerDiaMes },
      { name: 'p_fecha_hasta', type: 'D', value: hoy },
      { name: 'p_recaudadora_desde', type: 'I', value: 1 },
      { name: 'p_recaudadora_hasta', type: 'I', value: 999 }
    ])
    let arr = []

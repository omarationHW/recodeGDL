...
const OP_QUERY = 'sp_recuperacion_report'
...
const reload = async () => {
  showLoading('Cargando recuperaciones...', 'Consultando registros')
  currentPage.value = 1
  const t0 = performance.now()
  try {
    const response = await execute(OP_QUERY, BASE_DB, [
      { name: 'p_tipo', type: 'C', value: 'TODOS' }
    ])
    let arr = []

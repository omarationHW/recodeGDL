...
const OP_BUSCAR_MERCADOS = 'sp_buscar_mercados_adeudos'

const { loading, execute } = useApi()
const { showLoading, hideLoading } = useGlobalLoading()
const { showToast, handleApiError } = useLicenciasErrorHandler()
const rows=ref([]), cols=ref([]), currentPage=ref(1), itemsPerPage=ref(25)
...
const reload = async () => {
  showLoading('Cargando requerimientos...', 'Consultando registros')
  currentPage.value = 1
  const t0 = performance.now()
  try {
    const response = await execute(OP_BUSCAR_MERCADOS, BASE_DB, [
      { name: 'p_oficina', type: 'I', value: 1 },
      { name: 'p_num_mercado_desde', type: 'I', value: 1 },
      { name: 'p_num_mercado_hasta', type: 'I', value: 999 },
      { name: 'p_local_desde', type: 'I', value: 1 },
      { name: 'p_local_hasta', type: 'I', value: 999 },
      { name: 'p_seccion', type: 'C', value: null },
      { name: 'p_filtro_tipo', type: 'C', value: null },
      { name: 'p_filtro_valor', type: 'C', value: null },
      { name: 'p_adeudo_desde', type: 'D', value: 0 },
      { name: 'p_adeudo_hasta', type: 'D', value: 999999 },
      { name: 'p_user_id', type: 'I', value: 1 }
    ])
    let arr = []

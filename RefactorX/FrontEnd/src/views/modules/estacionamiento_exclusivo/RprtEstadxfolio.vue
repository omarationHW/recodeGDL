<template>
  <div class="module-view module-layout">
    ...
    <DocumentationModal
      ...
      title="Ayuda - RprtEstadxfolio"
    >
      <h3>Rprt Estadxfolio</h3>
...
const OP_QUERY = 'rpt_estadxfolio'
...
const buscar = async () => {
  showLoading('Generando reporte...', 'Estatus por folio')
  currentPage.value = 1
  searched.value = true
  const t0 = performance.now()
  try {
    const response = await execute(OP_QUERY, BASE_DB, [
      { name: 'p_modu', type: 'I', value: String(p_modu.value || '') },
      { name: 'p_rec', type: 'I', value: String(p_rec.value || '') },
      { name: 'p_fol1', type: 'I', value: String(p_fol1.value || '') },
      { name: 'p_fol2', type: 'I', value: String(p_fol2.value || '') }
    ])
    let arr = []
    ...
    showToast('success', `${rows.value.length} folio(s) en ${txt}`)
  } catch (e) {
    rows.value = []
    cols.value = []
    handleApiError(e)
...
const limpiar = () => { p_modu.value=''; p_rec.value=''; p_fol1.value=''; p_fol2.value=''; rows.value=[]; cols.value=[]; searched.value=false; currentPage.value=1 }

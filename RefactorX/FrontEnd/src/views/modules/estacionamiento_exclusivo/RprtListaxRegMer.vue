<template>
  <div class="module-view module-layout">
    ...
    <DocumentationModal
      ...
      title="Ayuda - RprtListaxRegMer"
    >
      <h3>Rprt Listax Reg Mer</h3>
    ...
    <TechnicalDocsModal
      :show="showTechDocs"
      :componentName="'RprtListaxRegMer'"
...
const OP_QUERY = 'rpt_listax_reg_mer'
...
const buscar = async () => {
  showLoading('Generando reporte...', 'Listado por mercados')
  currentPage.value = 1
  searched.value = true
  ...
  try {
    const response = await execute(OP_QUERY, BASE_DB, [
      ...
      { name: 'p_num_mercado', type: 'I', value: String(p_num_mercado.value || '') },
...
const limpiar = () => { p_vigencia.value=''; p_clave_practicado.value=''; p_num_mercado.value=''; p_oficina.value=''; rows.value=[]; cols.value=[]; searched.value=false; currentPage.value=1 }

<template>
  ...
  <DocumentationModal :show="showAyuda" :component-name="'AdeudosEnergia'" :module-name="'mercados'" :doc-type="'ayuda'" :title="'Mercados - AdeudosEnergia'" @close="showAyuda = false" />
  <DocumentationModal :show="showDocumentacion" :component-name="'AdeudosEnergia'" :module-name="'mercados'" :doc-type="'documentacion'" :title="'Mercados - AdeudosEnergia'" @close="showDocumentacion = false" />
</template>
...
// Toast removido - ahora usa useToast composable
...
const mostrarAyuda = () => {
  showToast('Ayuda: Seleccione una oficina y año para consultar los adeudos de energía eléctrica', 'info')
}
...
const buscar = async () => {
  if (!selectedOficina.value || !axo.value) {
    error.value = 'Debe seleccionar oficina y año'

<template>
  ...
  <DocumentationModal :show="showAyuda" :component-name="'AltaPagos'" :module-name="'mercados'" :doc-type="'ayuda'" :title="'Mercados - AltaPagos'" @close="showAyuda = false" />
  <DocumentationModal :show="showDocumentacion" :component-name="'AltaPagos'" :module-name="'mercados'" :doc-type="'documentacion'" :title="'Mercados - AltaPagos'" @close="showDocumentacion = false" />
</template>
...
async function mostrarLocalesDisponibles() {
  ...
  try {
    ...
    if (response.success) {
      localesDisponibles.value = response.data.result || [];
      ...
      showToast(response.message || 'Error al cargar locales', 'error');
    }
  } catch (error) {

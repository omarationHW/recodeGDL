        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">
              ...
              Registro de Pago - Local {{ localSeleccionado?.local }}
            </h5>
          ...
          <div class="modal-body">
            ...
            <div class="municipal-card mb-3">
              ...
              <div class="municipal-card-body">
                <div class="row">
                  ...
                  <div class="col-md-2">
                    <p class="mb-1"><strong>Clave Cuota:</strong> {{ localSeleccionado.clave_cuota }}</p>
                  </div>
                  <div class="col-md-3">
                ...
                <form @submit.prevent="verificarYAgregar">
                  <div class="form-row">
                    ...
                    <div class="form-group col-md-6">
                      ...
                      <input type="number" class="municipal-form-control" v-model.number="pago.periodo"
                        placeholder="1-12" min="1" max="12" :disabled="loading" required />
                    </div>
                  </div>
                  ...
                  <div class="form-row">
                    <div class="form-group col-md-6">
                      ...
                      <input type="number" class="municipal-form-control" v-model.number="pago.importePago"
            ...
            <!-- Historial de Pagos del Local -->
            <div v-if="pagosLocal.length > 0" class="municipal-card">
              <div class="municipal-card-header">
                <h6 class="mb-0">
                  <font-awesome-icon icon="history" />
                  Historial de Pagos ({{ pagosLocal.length }})
                </h6>
              </div>
              <div class="municipal-card-body">
                  ...
                  <table class="municipal-table table-sm">
                    ...
                    <tbody>
                      <tr v-for="(pagoHist, index) in pagosLocal" :key="index">
                        <td>{{ pagoHist.axo }}</td>
                        <td>{{ pagoHist.periodo }}</td>
                        <td>{{ formatFecha(pagoHist.fecha_pago) }}</td>
                        <td><strong>{{ formatMoneda(pagoHist.importe_pago) }}</strong></td>
            ...
            <div v-else class="alert alert-info">
              <font-awesome-icon icon="info-circle" />
              Este local no tiene pagos registrados aún.
            </div>
          </div>
...
async function cargarPagosLocal(idLocal) {
  try {
    const response = await axios.post('/api/generic', {
      eRequest: {
        Operacion: 'sp_alta_pagos_listar_pagos',
        Base: 'mercados',
        Esquema: 'publico',
        Parametros: [
...
async function agregarPago() {
  try {
    ...
    if (response.data?.eResponse?.success) {
      showToast('Pago agregado correctamente. El historial se ha actualizado.', 'success');
      limpiarFormularioPago();
...
async function verificarYModificar() {
  ...
  try {
    ...
    if (response.data?.eResponse?.success) {
      showToast('Pago modificado correctamente. El historial se ha actualizado.', 'success');
      limpiarFormularioPago();
      // Recargar pagos del local para actualizar el historial
      await cargarPagosLocal(localSeleccionado.value.id_local);
    }
...
function mostrarAyuda() {
  showToast('Ayuda: Filtre los locales, seleccione uno y registre los pagos correspondientes', 'info');
}
</script>

<template>
  ...
  <DocumentationModal :show="showAyuda" :component-name="'CveCuota'" :module-name="'mercados'" :doc-type="'ayuda'" :title="'Mercados - CveCuota'" @close="showAyuda = false" />
  <DocumentationModal :show="showDocumentacion" :component-name="'CveCuota'" :module-name="'mercados'" :doc-type="'documentacion'" :title="'Mercados - CveCuota'" @close="showDocumentacion = false" />
</template>
...
const cargarCveCuotas = async () => {
  ...
  try {
    ...
    if (response?.success) {
      cveCuotas.value = response.data.result || [];
    }
  } catch (error) {
  ...
  try {
    ...
    const response = await apiService.execute(
      ...
      [
        ...
        { nombre: 'p_descripcion', valor: form.value.descripcion, tipo: 'string' }
      ],
      '',
      null,
      'publico'

<template>
  ...
  <DocumentationModal :show="showAyuda" :component-name="'Menu'" :module-name="'mercados'" :doc-type="'ayuda'" :title="'Mercados - Menu'" @close="showAyuda = false" />
  <DocumentationModal :show="showDocumentacion" :component-name="'Menu'" :module-name="'mercados'" :doc-type="'documentacion'" :title="'Mercados - Menu'" @close="showDocumentacion = false" />
</template>
...
const verMeses = async (row) => {
  ...
  try {
    ...
    if (res.success) {
      mesesAdeudo.value = res.data.result || []
    } else {
      showToast(res.message || 'Error al cargar detalle de meses', 'error')
    }
  } catch (err) {

  ...
  <div class="module-view">
    <div class="module-view-header">
      ...
      <div class="module-view-info">
        <h1>Recargos Mantenimiento</h1>
        <p>Mercados - Recargos Mantenimiento</p>
      </div>
    </div>
    ...
    <form @submit.prevent="onSubmit">
      <div class="form-row">
        <div class="form-group col-md-2">
          ...
          <input type="number" v-model.number="form.axo" min="1994" max="2999" class="municipal-form-control" id="axo" required />
        </div>
        <div class="form-group col-md-2">
          <label class="municipal-form-label" for="periodo">Periodo</label>
          <input type="number" v-model.number="form.periodo" min="1" max="12" class="municipal-form-control" id="periodo" required />
        </div>
        <div class="form-group col-md-3">
          <label class="municipal-form-label" for="porcentaje">Porcentaje</label>
          <input type="number" v-model.number="form.porcentaje" step="0.01" min="0.01" max="99" class="municipal-form-control" id="porcentaje" required />
        </div>
      </div>
      <div class="form-row mt-3">
        <button type="submit" class="btn btn-municipal-primary mr-2">{{ editMode ? 'Actualizar' : 'Agregar' }}</button>
    ...
    <table class="-striped municipal-table-bordered mt-2">
      <thead>
        <tr>
          <th>Año</th>
          <th>Periodo</th>
      ...
      <tbody>
        <tr v-for="rec in recargos" :key="rec.axo + '-' + rec.periodo">
          ...
          <td>{{ rec.usuario }}</td>
          <td>
            <button class="btn-icon btn-municipal-info" @click="editRecargo(rec)">Editar</button>
          </td>
        </tr>

<template>
  <div class="module-view">
    ...
    <div class="module-view-content">
      ...
      <div v-if="!busquedaRealizada && !loading" class="municipal-alert municipal-alert-info"><font-awesome-icon icon="info-circle" /> Seleccione filtros y haga clic en <strong>Consultar</strong>.</div>
      ...
      <div v-if="results.length && !loading" class="municipal-card mt-3">
        <div class="municipal-card-header header-with-badge"><h5><font-awesome-icon icon="list-alt" /> Ingresos Locales</h5>
          <div class="header-right"><span class="badge-purple">{{ results.length }} registros</span><span class="badge-success ms-2">Total: {{ formatCurrency(totalImporte) }}</span></div>
        </div>
        <div class="municipal-card-body table-container">
          <div class="table-responsive">
            <table class="municipal-table">
              <thead class="municipal-table-header"><tr><th>Fecha</th><th>Mercado</th><th>Local</th><th>Nombre</th><th>Año/Periodo</th><th class="text-end">Importe</th></tr></thead>
              <tbody><tr v-for="(row, idx) in paginatedResults" :key="idx" class="row-hover"><td>{{ formatDate(row.fecha_pago) }}</td><td>{{ row.num_mercado }}</td><td>{{ datosLocal(row) }}</td><td>{{ row.nombre }}</td><td>{{ row.axo }}/{{ row.periodo }}</td><td class="text-end"><strong>{{ formatCurrency(row.importe_pago) }}</strong></td></tr></tbody>
              <tfoot class="municipal-table-footer"><tr><th colspan="5" class="text-end">TOTAL:</th><th class="text-end"><strong class="text-success">{{ formatCurrency(totalImporte) }}</strong></th></tr></tfoot>
            </table>
          </div>
...
const exportarExcel = () => { const data = results.value.map(row => ({ 'Fecha': formatDate(row.fecha_pago), 'Mercado': row.num_mercado, 'Local': datosLocal(row), 'Nombre': row.nombre, 'Año': row.axo, 'Periodo': row.periodo, 'Importe': row.importe_pago })); const csv = [Object.keys(data[0]).join(','), ...data.map(row => Object.values(row).join(','))].join('\n'); const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' }); const url = window.URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `ingresos_locales_${filters.value.oficina}_${filters.value.axo}.csv`; a.click(); window.URL.revokeObjectURL(url); };

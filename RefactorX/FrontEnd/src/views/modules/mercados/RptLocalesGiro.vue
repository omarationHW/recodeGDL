<template>
  ...
  <DocumentationModal :show="showAyuda" :component-name="'RptLocalesGiro'" :module-name="'mercados'" :doc-type="'ayuda'" :title="'Mercados - RptLocalesGiro'" @close="showAyuda = false" />
  <DocumentationModal :show="showDocumentacion" :component-name="'RptLocalesGiro'" :module-name="'mercados'" :doc-type="'documentacion'" :title="'Mercados - RptLocalesGiro'" @close="showDocumentacion = false" />
</template>
...
const fetchGiros = async () => { showLoading('Cargando giros...', 'Por favor espere'); loading.value = true; try { const response = await apiService.execute(
          'sp_get_giros',
          'mercados',
          [],
          '',
          null,
          'publico'
        ); if (response?.success && response?.data?.result) { giros.value = response.data.result; } } catch (error) { console.error('Error:', error); } finally { loading.value = false; hideLoading(); } };
const onOficinaChange = async () => { if (!filters.value.oficina) { mercados.value = []; filters.value.mercado = ''; return; } showLoading('Cargando mercados...', 'Por favor espere'); loading.value = true; try { const response = await apiService.execute(
          ...
          null,
          'publico'
        ); if (response?.success && response?.data?.result) { mercados.value = response.data.result; } else { mercados.value = []; } } catch (error) { console.error('Error:', error); mercados.value = []; } finally { loading.value = false; hideLoading(); } };
const consultar = async () => { if (!filters.value.oficina) { alert('Seleccione una recaudadora'); return; } showLoading('Consultando locales por giro...', 'Por favor espere'); loading.value = true; busquedaRealizada.value = false; try { const parametros = [{ nombre: 'p_oficina', valor: parseInt(filters.value.oficina) }]; if (filters.value.mercado) parametros.push({ nombre: 'p_mercado', valor: parseInt(filters.value.mercado) }); if (filters.value.giro) parametros.push({ nombre: 'p_giro', valor: parseInt(filters.value.giro) }); const response = await apiService.execute(
          'sp_rpt_locales_giro',
          'mercados',
          parametros,
          '',

  ...
  <div class="module-view">
    <div class="module-view-header">
      ...
      <div class="module-view-info">
        <h1>Descuento Multas - Transmisiones Patrimoniales</h1>
        <p>Multas y Reglamentos - Descuento de Multas para Transmisiones Patrimoniales</p>
      </div>
    ...
    <div class="module-view-content">
      <div class="municipal-card">
        <div class="municipal-card-header" @click="toggleFilters" style="cursor: pointer;">
          <h5>
            <font-awesome-icon icon="filter" /> Busqueda de Transmision
      ...
      <div class="municipal-card">
        <div class="municipal-card-header header-with-badge">
          <h5><font-awesome-icon icon="file-alt" /> Datos de la Transmision</h5>
        </div>
        ...
        <div class="municipal-card-body">
          <div v-if="!searchPerformed" class="empty-state">
            <div class="empty-state-content">
              ...
              <p class="empty-state-text">Ingrese el folio de la transmision patrimonial</p>
            </div>
          </div>

          <div v-else-if="!transmisionData" class="empty-state">
            <div class="empty-state-content">
              <font-awesome-icon icon="search" class="empty-state-icon" />
              <p class="empty-state-text">No se encontro la transmision o ya fue pagada/cancelada</p>
            </div>
          </div>
          ...
          <div v-else>
            <div class="details-grid mb-4">
              <div class="detail-section">
                <h6 class="section-title"><font-awesome-icon icon="file-invoice" /> Datos Generales</h6>
                <table class="detail-table">
                  <tr><td class="label">Folio:</td><td><strong class="text-primary">{{ transmisionData.folio }}</strong></td></tr>
                  <tr><td class="label">Cuenta:</td><td>{{ transmisionData.cuenta }}</td></tr>
                  <tr><td class="label">Fecha Transmision:</td><td>{{ formatDate(transmisionData.fecha_transmision) }}</td></tr>
                  <tr><td class="label">Adquiriente:</td><td>{{ transmisionData.adquiriente || 'N/A' }}</td></tr>
                </table>
              </div>
              <div class="detail-section">
                <h6 class="section-title"><font-awesome-icon icon="dollar-sign" /> Importes</h6>
                <table class="detail-table">
                  <tr><td class="label">Impuesto:</td><td class="text-end">{{ formatCurrency(transmisionData.impuesto) }}</td></tr>
                  <tr><td class="label">Recargos:</td><td class="text-end">{{ formatCurrency(transmisionData.recargos) }}</td></tr>
                  <tr><td class="label">Multas:</td><td class="text-end"><strong class="text-danger">{{ formatCurrency(transmisionData.multas) }}</strong></td></tr>
                  <tr><td class="label">Total:</td><td class="text-end"><strong>{{ formatCurrency(transmisionData.total) }}</strong></td></tr>
                </table>
              </div>
            </div>

            <div v-if="transmisionData.descuento_vigente" class="alert alert-info mb-4">
              <font-awesome-icon icon="info-circle" class="me-2" />
              <strong>Descuento vigente:</strong> {{ transmisionData.descuento_porcentaje }}%
              (Aplicado el {{ formatDate(transmisionData.descuento_fecha) }})
            </div>

            <div v-if="transmisionData.multas > 0 && !transmisionData.descuento_vigente" class="municipal-card">
    ...
    <DocumentationModal :show="showDocModal" :componentName="'DescMultaTransFrm'" :moduleName="'multas_reglamentos'"
      :docType="docType" :title="'Descuento Multas Transmisiones'" @close="showDocModal = false" />
  </div>
</template>
...
const transmisionData = ref(null)
...
const buscar = async () => {
  ...
  showLoading('Buscando transmision...', 'Consultando base de datos')
  searchPerformed.value = true
  descuento.value = { porcentaje: 0, funcionario: '' }

  try {
    const response = await execute('sp_descmultatrans_consulta', 'multas_reglamentos',
      [{ nombre: 'p_folio', valor: parseInt(filters.value.folio), tipo: 'integer' }],
      'guadalajara', null, 'publico')

    if (response?.result?.[0]) {
      transmisionData.value = response.result[0]
      showToast('success', 'Transmision encontrada')
    } else {
      transmisionData.value = null
      showToast('info', 'No se encontro la transmision')
    }
  } catch (error) {
    handleApiError(error, 'Error al buscar transmision')
    transmisionData.value = null
  } finally {
    hideLoading()
  }
...
const aplicarDescuento = async () => {
  ...
  const confirm = await Swal.fire({
    ...
    html: `<p>Se aplicara descuento del <strong>${descuento.value.porcentaje}%</strong> a la transmision <strong>${transmisionData.value.folio}</strong></p>`,
  ...
  try {
    const response = await execute('sp_descmultatrans_aplicar', 'multas_reglamentos',
      [
        { nombre: 'p_folio', valor: transmisionData.value.folio, tipo: 'integer' },
    ...
    if (response?.result?.[0]?.success) {
      await Swal.fire({ icon: 'success', title: 'Descuento aplicado', text: `Folio: ${response.result[0].folio_descuento || 'N/A'}`, confirmButtonColor: '#ea8215' })
      buscar()
    } else {

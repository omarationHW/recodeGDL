<template>
  <div class="module-view module-layout">
    <div class="module-view-header"><div class="module-view-icon"><font-awesome-icon icon="dumpster" /></div><div class="module-view-info"><h1>Derechos de Fosa</h1><p>Consulta y control</p></div></div>
    <div class="module-view-content">
      <div class="municipal-card"><div class="municipal-card-body">
        <div class="form-row"><div class="form-group"><label class="municipal-form-label">Folio</label><input class="municipal-form-control" v-model.number="filters.folio" type="number"/></div></div>
...
const OP_LIST='RECAUDADORA_DRECGO_FOSA' // TODO confirmar
const { loading, execute } = useApi()
const filters=ref({ folio:null })
const rows=ref([])
const columns=ref([])
async function reload(){ const params=[{name:'folio',type:'I',value:Number(filters.value.folio||0)}]; try{ const data=await execute(OP_LIST,BASE_DB,params); const arr=Array.isArray(data?.rows)?data.rows:Array.isArray(data)?data:[]; rows.value=arr; columns.value=arr.length?Object.keys(arr[0]):[] }catch(e){ rows.value=[]; columns.value=[] } }
reload()
</script>

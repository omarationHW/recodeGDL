          ...
          <div v-else-if="Array.isArray(result) && result.length > 0">
            ...
            <div v-else class="alert-success">
              ...
              <div class="result-details" v-if="result[0].folio_requerimiento">
                <p>Folio del Requerimiento: <strong>{{ result[0].folio_requerimiento }}</strong></p>
                <p>Clave Catastral: <strong>{{ filters.cvecat }}</strong></p>
                <p v-if="result[0].fecha_generacion">Fecha de Generación: <strong>{{ result[0].fecha_generacion }}</strong></p>
                <p v-if="result[0].propietario">Propietario: <strong>{{ result[0].propietario }}</strong></p>
                <p v-if="result[0].direccion">Dirección: <strong>{{ result[0].direccion }}</strong></p>
                <p v-if="result[0].monto_adeudo">Monto de Adeudo: <strong>{{ formatCurrency(result[0].monto_adeudo) }}</strong></p>
              </div>
            </div>
          </div>

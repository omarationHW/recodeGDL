<?php
// Buscar tablas relacionadas con descuentos de multa en la BD multas_reglamentos
...
try {
    ...
    echo "=== BUSCANDO TABLAS CON 'DESC', 'MULTA', 'DESCTO' ===\n\n";
    ...
    $stmt = $pdo->query("
        ...
        AND (tablename ILIKE '%desc%multa%'
             OR tablename ILIKE '%multa%desc%'
             OR tablename ILIKE '%descto%'
             OR tablename ILIKE '%descuento%')
    ...
    if (count($tablas) > 0) {
        foreach ($tablas as $tabla) {
            ...
            // Ejemplos de datos (solo si tiene menos de 50k registros)
            try {
                $count_stmt = $pdo->query("SELECT COUNT(*) as total FROM {$tabla['schemaname']}.{$tabla['tablename']}");
                $count = $count_stmt->fetch(PDO::FETCH_ASSOC);

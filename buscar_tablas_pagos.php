<?php
// Buscar tablas relacionadas con pagos, recibos, transacciones en la BD multas_reglamentos
...
try {
    ...
    echo "=== BUSCANDO TABLAS CON 'PAG', 'RECIB', 'TRANS', 'CAJA' ===\n\n";
    ...
    $stmt = $pdo->query("
        ...
        AND (tablename ILIKE '%pag%' OR tablename ILIKE '%recib%'
             OR tablename ILIKE '%trans%' OR tablename ILIKE '%caja%'
             OR tablename ILIKE '%banco%')
    ...
    if (count($tablas) > 0) {
        foreach ($tablas as $tabla) {
            ...
            // Ejemplos de datos (solo para tablas con menos de 100k registros)
            try {
                $count_stmt = $pdo->query("SELECT COUNT(*) as total FROM {$tabla['schemaname']}.{$tabla['tablename']}");
                $count = $count_stmt->fetch(PDO::FETCH_ASSOC);

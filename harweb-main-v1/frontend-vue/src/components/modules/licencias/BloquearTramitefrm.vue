  ...
  methods: {
    ...
    async buscarTramite() {
      ...
      try {
        ...
        if (tramiteResponse.data.eResponse && tramiteResponse.data.eResponse.success && tramiteResponse.data.eResponse.data.result && tramiteResponse.data.eResponse.data.result.length > 0) {
          ...
          // Cargar historial después de encontrar el trámite
    ...
    async cargarHistorial() {
      ...
      try {
        ...
        const eRequest = {
          Operacion: 'sp_consultar_historial_tramite_paginado',
          Base: 'licencias',
          Parametros: [
        ...
        if (response.data.eResponse && response.data.eResponse.success) {
          ...
          // Filtrar bloqueos activos después de cargar historial
          this.actualizarBloqueosActivos();
        }
    ...
    actualizarBloqueosActivos() {
      // Necesitamos cargar todos los registros para determinar bloqueos activos
      // Esta función se ejecuta después de cargar el historial

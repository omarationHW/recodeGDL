    ...
    async onSubmit() {
      ...
      try {
        ...
        if (response.data && response.data.eResponse.success) {
          this.reportData = response.data.eResponse.data.result;
          this.message = response.data.eResponse.data.result.length > 0 ?
            `Se encontraron ${response.data.eResponse.data.result.length} licencias con adeudos desde el año ${this.year}` :
            'No se encontraron licencias con adeudos para el año especificado';
        } else {
          this.error = response.data?.eResponse?.message || 'Error al obtener el reporte.';
        }
      } catch (err) {

-- Stored Procedure: sp_adeudos_mult_ins_insert
-- Tipo: CRUD
...
-- Generado para formulario: AdeudosMult_Ins
-- Fecha: 2025-08-27 13:39:07
...
BEGIN
    FOR r IN SELECT * FROM jsonb_array_elements(p_rows) LOOP
        ...
        JOIN ta_16_unidades c ON c.cve_recolec = b.cve_recolec AND c.ejercicio_recolec = p_anio
        ...
        SELECT 1 INTO v_cuota_normal FROM ta_16_pagos WHERE control_contrato = v_control_contrato AND aso_mes_pago = p_anio||'-'||p_mes AND ctrol_operacion = 6 AND (status_vigencia = 'V' OR status_vigencia = 'P') LIMIT 1;
        ...
        SELECT 1 INTO v_exed FROM ta_16_pagos WHERE control_contrato = v_control_contrato AND aso_mes_pago = p_anio||'-'||p_mes AND ctrol_operacion = p_tipo_oper LIMIT 1;
        ...
        INSERT INTO ta_16_pagos (control_contrato, aso_mes_pago, ctrol_operacion, importe, status_vigencia, fecha_hora_pago, id_rec, caja, consec_operacion, folio_rcbo, usuario, exedencias)

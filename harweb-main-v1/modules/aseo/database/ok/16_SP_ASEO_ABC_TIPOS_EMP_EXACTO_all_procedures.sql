...
-- Archivo: 16_SP_ASEO_ABC_TIPOS_EMP_EXACTO_all_procedures.sql
-- Generado: 2025-09-09
-- Total SPs: 5 (EXACTO)
-- ============================================
...
RETURNS TABLE(ctrol_emp integer, tipo_empresa varchar, descripcion varchar) AS $$
BEGIN
  RETURN QUERY SELECT ctrol_emp, tipo_empresa, descripcion FROM public.ta_16_tipos_emp WHERE ctrol_emp = p_ctrol_emp;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  IF v_exists = 0 THEN
    ...
    RETURN;
  END IF;
  UPDATE public.ta_16_tipos_emp SET tipo_empresa = p_tipo_empresa, descripcion = p_descripcion

...
-- Formulario: Adeudos_Ins (EXACTO del archivo original)
-- Archivo: 26_SP_ASEO_ADEUDOS_INS_EXACTO_all_procedures.sql
-- Generado: 2025-09-09
-- Total SPs: 3 (EXACTO)
-- ============================================
...
BEGIN
    ...
    SELECT 1 INTO v_exists FROM public.ta_16_pagos WHERE control_contrato = v_control_contrato AND aso_mes_pago = v_fecha_cap AND ctrol_operacion = 6 AND (status_vigencia = 'V' OR status_vigencia = 'P') LIMIT 1;
    IF NOT FOUND THEN
        RETURN QUERY SELECT false, 'No existe cuota normal para el periodo';
        RETURN;
    END IF;
    -- Validar que no exista exedencia
    SELECT 1 INTO v_exists FROM public.ta_16_pagos WHERE control_contrato = v_control_contrato AND aso_mes_pago = v_fecha_cap AND ctrol_operacion = p_ctrol_operacion AND status_vigencia <> 'Z' LIMIT 1;
    IF FOUND THEN
...
BEGIN
    RETURN QUERY SELECT
        (SELECT jsonb_agg(row_to_json(t)) FROM (SELECT ctrol_aseo, tipo_aseo, descripcion FROM public.cat_tipo_aseo ORDER BY ctrol_aseo) t),
        (SELECT jsonb_agg(row_to_json(t)) FROM (SELECT ctrol_operacion, cve_operacion, descripcion FROM public.cat_operacion WHERE ctrol_operacion > 6 ORDER BY ctrol_operacion) t),

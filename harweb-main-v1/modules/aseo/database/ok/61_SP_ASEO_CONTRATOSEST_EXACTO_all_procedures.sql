...
-- Formulario: CONTRATOSEST (EXACTO del archivo original)
-- Archivo: 61_SP_ASEO_CONTRATOSEST_EXACTO_all_procedures.sql
-- Generado: 2025-09-10
-- Total SPs: 1 (EXACTO)
-- ============================================

-- SP 1/1: sp_contratos_estadistico
-- Tipo: Report
...
BEGIN
    ...
    LEFT JOIN public.ta_16_pagos a ON a.control_contrato = c.control_contrato
    WHERE
        ...
        AND (
            (p_adeudos_visual = 0 AND EXISTS (SELECT 1 FROM public.ta_16_pagos ap WHERE ap.control_contrato = c.control_contrato))
            OR (p_adeudos_visual = 1 AND NOT EXISTS (SELECT 1 FROM public.ta_16_pagos ap WHERE ap.control_contrato = c.control_contrato))
        )

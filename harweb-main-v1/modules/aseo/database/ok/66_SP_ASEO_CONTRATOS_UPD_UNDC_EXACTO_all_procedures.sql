...
-- Formulario: CONTRATOS_UPD_UNDC (EXACTO del archivo original)
-- Archivo: 66_SP_ASEO_CONTRATOS_UPD_UNDC_EXACTO_all_procedures.sql
-- Generado: 2025-09-10
-- Total SPs: 3 (EXACTO)
-- ============================================
...
BEGIN
    -- Validaciones
    SELECT * INTO v_contrato FROM public.ta_16_contratos WHERE control_contrato = p_control_contrato AND status_vigencia = 'V';
    IF NOT FOUND THEN
    ...
    SELECT * INTO v_unidad FROM public.ta_16_unidades WHERE ejercicio_recolec = p_ejercicio AND cve_recolec = v_contrato.cve_recolec LIMIT 1;
    IF NOT FOUND THEN
        RETURN QUERY SELECT FALSE, 'No existe tarifa de unidad para el ejercicio actual.';
        RETURN;
    END IF;
    -- Buscar unidad siguiente
    SELECT * INTO v_unidad_sig FROM public.ta_16_unidades WHERE ejercicio_recolec = v_ejercicio_sig AND cve_recolec = v_contrato.cve_recolec LIMIT 1;
    -- Actualizar contrato
    UPDATE public.ta_16_contratos SET cantidad_recolec = p_nueva_cantidad, usuario = p_usuario WHERE control_contrato = p_control_contrato;
    -- Actualizar pagos del ejercicio actual
    v_importe := p_nueva_cantidad * v_unidad.costo_unidad;
    UPDATE public.ta_16_pagos SET exedencias = p_nueva_cantidad, importe = v_importe
    ...
    IF FOUND AND v_unidad_sig IS NOT NULL THEN
        ...
        UPDATE public.ta_16_pagos SET importe = v_importe_sig
          WHERE control_contrato = p_control_contrato

...
-- Archivo: 05_SP_CEMENTERIOS_CONINDIVIDUAL_EXACTO_all_procedures.sql
-- Generado: 2025-09-09
-- Total SPs: 11 (EXACTO)
-- ============================================
...
AS $$
BEGIN
  SELECT * FROM public.ta_12_passwords WHERE id_usuario = usuario;
END;
$$;
...
AS $$
BEGIN
  SELECT * FROM public.ta_13_datosrcmadic WHERE control_rcm = control_rcm;
END;
$$;
...
AS $$
BEGIN
  SELECT d.*, p.nombre FROM public.ta_13_descrec d
  JOIN public.ta_12_passwords p ON d.usuario_alta = p.usuario
...
AS $$
BEGIN
  SELECT * FROM public.tc_13_cementerios WHERE cementerio = cementerio;
END;
$$;
...
BEGIN
  ...
  FROM public.spd_13_constot(par_control, par_axo);
END;
$$;

...
-- Formulario: Descuentos (EXACTO del archivo original)
-- Archivo: 09_SP_CEMENTERIOS_DESCUENTOS_EXACTO_all_procedures.sql
-- Generado: 2025-09-09
-- Total SPs: 1 (EXACTO)
-- ============================================

-- SP 1/1: spd_13_abcdesctos
-- Tipo: CRUD
-- Descripción: Alta, baja, modificación y reactivación de descuentos para public. OPC: 1=alta, 2=baja, 3=modifica, 4=reactivar.
-- --------------------------------------------
...
BEGIN
    IF v_opc = 1 THEN -- Alta
        SELECT COUNT(*) INTO v_exists FROM public.ta_13_descpens WHERE control_rcm = v_control AND axo_descto = v_axo AND vigencia = 'V';
        IF v_exists > 0 THEN
            par_ok := 1;
            par_observ := 'Ya tiene descuento para el adeudo del año seleccionado, verifique';
            RETURN NEXT;
        END IF;
        INSERT INTO public.ta_13_descpens (control_rcm, axo_descto, descuento, usuario, fecha_alta, vigencia, reactivar, tipo_descto)
    ...
    ELSIF v_opc = 2 THEN -- Baja
        UPDATE public.ta_13_descpens SET vigencia = 'B', usuario_mov = v_usu, fecha_mov = CURRENT_DATE
    ...
    ELSIF v_opc = 3 THEN -- Modifica
        UPDATE public.ta_13_descpens SET descuento = v_porc, usuario_mov = v_usu, fecha_mov = CURRENT_DATE
        WHERE control_rcm = v_control AND axo_descto = v_axo AND vigencia = 'V';
        par_ok := 0;
        par_observ := 'Descuento modificado correctamente';
        RETURN NEXT;
    ELSIF v_opc = 4 THEN -- Reactivar
        UPDATE public.ta_13_descpens SET reactivar = 'S', usuario_mov = v_usu, fecha_mov = CURRENT_DATE

...
BEGIN
    ...
    SELECT COALESCE(MAX(axo_pago_hasta), 0) INTO v_uap FROM public.ta_13_pagosrcm WHERE control_rcm = p_folio_a;
    IF v_uap = 0 THEN
        v_uap := v_axo - 5;
    END IF;
    UPDATE public.ta_13_datosrcm SET axo_pagado = v_uap WHERE control_rcm = p_folio_a;
    -- UAP para folio origen
    SELECT COALESCE(MAX(axo_pago_hasta), 0) INTO v_uap FROM public.ta_13_pagosrcm WHERE control_rcm = p_folio_de;
    IF v_uap = 0 THEN
        v_uap := v_axo - 5;
    END IF;
    UPDATE public.ta_13_datosrcm SET axo_pagado = v_uap WHERE control_rcm = p_folio_de;
    IF v_suma > 0 THEN

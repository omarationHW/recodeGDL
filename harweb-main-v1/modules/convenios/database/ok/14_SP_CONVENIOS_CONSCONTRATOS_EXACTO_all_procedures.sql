...
-- Archivo: 14_SP_CONVENIOS_CONSCONTRATOS_EXACTO_all_procedures.sql
-- Generado: 2025-09-10
-- Total SPs: 4 (EXACTO)
-- ============================================
...
-- SP 2/4: sp_cons_contratos_search_by_nombre
-- Tipo: Report
-- Descripción: Busca contratos por nombre (LIKE)
-- --------------------------------------------

CREATE OR REPLACE FUNCTION sp_cons_contratos_search_by_nombre(p_nombre varchar)
RETURNS TABLE(
    id_convenio integer,
    colonia smallint,
    calle smallint,
...
BEGIN
    ...
    ORDER BY nombre ASC, colonia ASC, calle ASC, folio ASC;
END;
$$ LANGUAGE plpgsql;
...
-- SP 4/4: sp_cons_contratos_get_detalle
-- Tipo: Report
...
BEGIN
    RETURN QUERY
    SELECT * FROM public.ta_17_convenios WHERE id_convenio = p_id_convenio;
END;
$$ LANGUAGE plpgsql;

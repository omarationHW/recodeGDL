...
-- Formulario: deudagrupo (EXACTO del archivo original)
-- Archivo: 17_SP_CONVENIOS_DEUDAGRUPO_EXACTO_all_procedures.sql
-- Generado: 2025-09-10
-- Total SPs: 1 (EXACTO)
-- ============================================
...
BEGIN
    ...
    FOR r IN
        SELECT c.axo_obra as obra, a.id_convenio, a.colonia, a.calle, a.folio, a.nombre, a.pago_total, a.fecha_vencimiento,
               ...
               (SELECT COALESCE(SUM(g.importe),0) FROM public.ta_17_convenios h, public.ta_17_pagos f, public.ta_12_recibosdet g
        ...
        HAVING (a.pago_total - (COALESCE(SUM(b.importe),0) +
                ...
                (SELECT COALESCE(SUM(importe),0) FROM public.ta_17_devoluciones WHERE id_convenio=a.id_convenio)
               ) > 0

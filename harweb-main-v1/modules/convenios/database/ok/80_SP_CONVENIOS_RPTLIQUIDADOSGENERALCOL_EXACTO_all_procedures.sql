...
-- Formulario: RPTLIQUIDADOSGENERALCOL (EXACTO del archivo original)
-- Archivo: 80_SP_CONVENIOS_RPTLIQUIDADOSGENERALCOL_EXACTO_all_procedures.sql
-- Generado: 2025-09-10
-- Total SPs: 1 (EXACTO)
-- ============================================
    ...
    HAVING (
        a.pago_total - (
            ...
            + (
                ...
                JOIN public.ta_17_pagos f ON h.id_convenio = a.id_convenio AND f.id_convenio = h.id_convenio AND h.clave_historia = 2
                JOIN public.ta_12_recibosdet g ON g.fecha = f.fecha_pago AND g.id_rec = f.oficina_pago AND g.caja = f.caja_pago AND g.operacion = f.operacion_pago AND g.cuenta IN (46210, 570200000)
            )
            - (
                SELECT COALESCE(SUM(importe), 0)
                FROM public.ta_17_devoluciones

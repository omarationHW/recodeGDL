...
-- Formulario: bajaAnunciofrm (EXACTO del archivo original)
-- Archivo: 02_SP_LICENCIAS_BAJAANUNCIOFRM_EXACTO_all_procedures.sql
-- Generado: 2025-09-10
-- Total SPs: 3 (EXACTO)
-- ============================================
...
BEGIN
  SELECT a.id_anuncio, a.id_licencia, a.fecha_otorgamiento, l.propietario,
         ...
         (
           SELECT jsonb_agg(row_to_json(s)) FROM (
             SELECT axo, saldo FROM public.detsal_lic WHERE id_anuncio = a.id_anuncio AND cvepago = 0
           ) s
         ) AS adeudos
  ...
  LEFT JOIN public.licencias l ON l.id_licencia = a.id_licencia
...
BEGIN
  RETURN QUERY
    ...
    JOIN public.deptos d ON d.cvedepto = u.cvedepto
...
BEGIN
  SELECT vigente, id_licencia INTO v_vigente, v_id_licencia FROM public.anuncios WHERE anuncio = p_anuncio;
  ...
  -- Cancelar adeudos
  UPDATE public.detsal_lic SET cvepago = 999999 WHERE id_anuncio = p_anuncio AND cvepago = 0;

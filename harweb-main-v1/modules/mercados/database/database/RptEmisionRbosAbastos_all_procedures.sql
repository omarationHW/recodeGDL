...
BEGIN
    FOR r IN
        SELECT a.id_local, a.oficina, a.num_mercado, a.categoria, a.seccion, a.local, a.letra_local, a.bloque, a.nombre, b.descripcion, a.descripcion_local,
               ...
               COALESCE(SUM(ROUND((d.importe * (
                   SELECT SUM(porcentaje_mes) FROM ta_12_recargos WHERE (axo = d.axo AND mes >= d.periodo) OR (axo > d.axo)
               ))/100,2)),0) AS recargos,
        ...
        LEFT JOIN ta_11_adeudo_local d ON a.id_local = d.id_local
    ...
    LOOP
        ...
        IF r.seccion = 'PS' THEN
            r.renta := (r.importe_cuota * COALESCE((SELECT superficie FROM ta_11_locales WHERE id_local = r.id_local),1)) * 30;
        ELSE
            r.renta := COALESCE((SELECT superficie FROM ta_11_locales WHERE id_local = r.id_local),1) * r.importe_cuota;
        END IF;
        -- Calcular rentaaxos y meses adeudados
        cad := '';
        rentaaxos := 0;
        FOR m IN SELECT periodo, importe, axo FROM ta_11_adeudo_local WHERE id_local = r.id_local AND axo = r.axo_1 LOOP
            IF m.axo < p_axo THEN

...
BEGIN
    ...
    FROM public.ta_cheques a
    JOIN public.ta_cuentas b ON a.moneda = b.moneda AND a.cta_mayor = b.cta_mayor AND a.cta_sub01 = b.cta_sub01 AND a.cta_sub02 = b.cta_sub02 AND a.cta_sub03 = b.cta_sub03
    JOIN public.ta_proveedores p ON a.id_proveedor = p.id_proveedor
...
BEGIN
    RETURN QUERY
    SELECT nombre FROM public.ta_cuentastrans
...
BEGIN
    RETURN QUERY
    SELECT banco, nombre, pagador FROM public.ta_bancopagador WHERE pagador = 'S';
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    SELECT nombre INTO cuenta_trans FROM public.ta_cuentastrans
    ...
    FOR doc IN SELECT * FROM json_to_recordset(p_documentos) AS (
        ...
        ELSIF p_tipo_doc = 'P' THEN
            -- Elec. Bco. Pagador
            SELECT cuenta INTO txtbenef FROM public.ta_proveedores WHERE id_proveedor = doc.id_proveedor;
        ...
        ELSIF p_tipo_doc = 'O' THEN
            -- Elec. Otros Bancos
            SELECT cuenta, banco, plaza_num, sucursal INTO txtbenef, banco_dest, plaza_num, sucursal FROM public.ta_proveedores WHERE id_proveedor = doc.id_proveedor;
            txt_line := LPAD(txtbenef, 18, '0') ||
                        LPAD(TRIM(cuenta_trans), 18, '0') ||

...
BEGIN
    ...
    JOIN public.ta_11_cuo_locales c ON c.axo = EXTRACT(YEAR FROM CURRENT_DATE) AND c.categoria = l.categoria AND c.seccion = l.seccion AND c.clave_cuota = l.clave_cuota
    JOIN public.ta_11_mercados m ON m.oficina = l.oficina AND m.num_mercado_nvo = l.num_mercado
    LEFT JOIN public.ta_11_fecha_desc fd ON fd.mes = EXTRACT(MONTH FROM CURRENT_DATE)
...
BEGIN
    ...
    JOIN public.ta_11_locales l ON l.id_local = m.id_local
...
BEGIN
    ...
    JOIN public.ta_11_locales l ON l.id_local = p.id_local
...
BEGIN
    ...
    JOIN public.ta_12_importes i ON a.fecing = i.fecing AND a.recing = i.recing AND a.cajing = i.cajing AND a.opcaja = i.opcaja
    JOIN public.ta_11_mercados m ON i.cta_aplicacion = m.cuenta_ingreso
    JOIN public.ta_12_zonas z ON m.id_zona = z.id_zona

...
BEGIN
    ...
    SELECT 
        ...
        END AS renta,
        -- Leyenda y adeudo
        CASE WHEN (SELECT COUNT(1) FROM public.ta_11_adeudo_local al WHERE al.id_local = l.id_local AND ((al.axo = p_axo AND al.periodo <= p_mes) OR (al.axo < p_axo))) >= 1 THEN 'Local con Adeudo' ELSE 'Local al Corriente de Pagos' END AS leyenda,
        CASE WHEN (SELECT COUNT(1) FROM public.ta_11_adeudo_local al WHERE al.id_local = l.id_local AND ((al.axo = p_axo AND al.periodo <= p_mes) OR (al.axo < p_axo))) >= 1 THEN 1 ELSE 0 END AS adeudo

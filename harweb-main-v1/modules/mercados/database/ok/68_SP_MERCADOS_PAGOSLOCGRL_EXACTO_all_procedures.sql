...
-- Formulario: PagosLocGrl
-- Generado: 2025-08-27 00:26:11
-- Total SPs: 3
-- ============================================

-- SP 1/3: get_recaudadoras
-- Tipo: Catalog
...
BEGIN
  RETURN QUERY SELECT id_rec, recaudadora, domicilio, tel, recaudador FROM public.ta_12_recaudadoras ORDER BY id_rec;
END;
$$ LANGUAGE plpgsql;

-- ============================================

-- SP 2/3: get_mercados_by_recaudadora
-- Tipo: Catalog
-- Descripción: Devuelve los mercados de una public.
-- --------------------------------------------
...
BEGIN
  RETURN QUERY SELECT num_mercado_nvo, descripcion, categoria, cuenta_ingreso FROM public.ta_11_mercados WHERE oficina = rec_id ORDER BY num_mercado_nvo;
END;
$$ LANGUAGE plpgsql;

-- ============================================

-- SP 3/3: get_pagos_loc_grl
-- Tipo: Report
...
BEGIN
  ...
  SELECT 
    ...
    (
      ...
      FROM public.ta_15_periodos p WHERE p.control_otr = a.id_local
    ) as requerimientos,
    a.id_local
  FROM public.ta_11_locales a
  JOIN public.ta_11_pagos_local b ON b.id_local = a.id_local
  JOIN public.ta_12_passwords c ON c.id_usuario = b.id_usuario
  LEFT JOIN public.ta_15_apremios d ON d.modulo = 11 AND d.control_otr = b.id_local AND d.vigencia = '2' AND d.fecha_pago = b.fecha_pago AND d.recaudadora = b.oficina_pago AND d.caja = b.caja_pago AND d.operacion = b.operacion_pago

...
DECLARE
    ...
    mesesade INT;
    mesrec RECORD;
    qmes CURSOR FOR SELECT periodo, importe FROM public.ta_11_adeudo_local WHERE id_local = r.id_local AND axo = p_axo ORDER BY periodo;
BEGIN
    FOR r IN
        ...
        JOIN public.ta_12_recaudadoras d ON c.oficina = d.id_rec
    ...
    LOOP
        ...
        renta := 0;
        mesesade := 0;
        FOR mesrec IN SELECT periodo, importe FROM public.ta_11_adeudo_local WHERE id_local = r.id_local AND axo = p_axo AND periodo <= p_periodo ORDER BY periodo LOOP

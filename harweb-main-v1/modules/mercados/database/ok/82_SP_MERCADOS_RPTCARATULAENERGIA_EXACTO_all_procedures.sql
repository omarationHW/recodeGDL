...
BEGIN
    ...
    JOIN public.ta_11_locales l ON l.id_local = e.id_local
    JOIN public.ta_11_mercados m ON m.oficina = l.oficina AND m.num_mercado_nvo = l.num_mercado
    JOIN public.ta_12_passwords u ON u.id_usuario = e.id_usuario
...
BEGIN
    ...
    FROM public.ta_11_adeudo_energ a WHERE a.id_adeudo_energia = p_id_adeudo;
    SELECT e.id_local INTO v_id_local FROM public.ta_11_energia e WHERE e.id_energia = v_id_energia;
    SELECT l.num_mercado INTO v_mercado FROM public.ta_11_locales l WHERE l.id_local = v_id_local;
    ...
    FOR v_cont IN 1..v_meses LOOP
        ...
        SELECT COUNT(*) INTO v_req_count FROM public.ta_15_apremios WHERE modulo=33 AND control_otr=v_id_local;
        IF v_axo = v_alo THEN

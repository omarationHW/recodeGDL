...
-- Formulario: Apremios (EXACTO del archivo original)
-- Archivo: 01_SP_OTRASOBLIG_APREMIOS_EXACTO_all_procedures.sql
-- Generado: 2025-09-10
-- Total SPs: 5 (EXACTO)
-- ============================================
...
BEGIN
    ...
    FROM otrasoblig.ta_15_apremios a
    JOIN otrasoblig.ta_12_passwords b ON b.id_usuario = a.ejecutor
...
BEGIN
    ...
    FROM otrasoblig.ta_15_periodos
...
CREATE OR REPLACE FUNCTION sp_create_apremio(
    ...
    modulo integer,
    control_otr integer
) RETURNS SETOF otrasoblig.ta_15_apremios AS $$
DECLARE
    new_id integer;
BEGIN
    INSERT INTO otrasoblig.ta_15_apremios (
    ...
    RETURN QUERY SELECT * FROM otrasoblig.ta_15_apremios WHERE id_control = new_id;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    UPDATE otrasoblig.ta_15_apremios SET
        zona = zona,
        folio = folio,
    ...
    RETURN QUERY SELECT * FROM otrasoblig.ta_15_apremios WHERE id_control = id_control;
END;
$$ LANGUAGE plpgsql;
...
RETURNS VOID AS $$
BEGIN
    UPDATE otrasoblig.ta_15_apremios SET vigencia = '0' WHERE id_control = id_control;
END;
$$ LANGUAGE plpgsql;

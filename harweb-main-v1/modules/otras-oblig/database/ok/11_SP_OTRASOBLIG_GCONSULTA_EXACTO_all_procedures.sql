...
-- Formulario: GConsulta (EXACTO del archivo original)
-- Archivo: 11_SP_OTRASOBLIG_GCONSULTA_EXACTO_all_procedures.sql
-- Generado: 2025-09-10
-- Total SPs: 8 (EXACTO)
-- ============================================
...
BEGIN
    ...
    FROM otrasoblig.t34_adeudos_totales
    WHERE cve_tab = par_tabla AND id_datos = par_id_datos AND axo = par_aso AND mes = par_mes;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    FROM otrasoblig.admin_adeudos_detalle
...
BEGIN
    ...
    FROM otrasoblig.admin_adeudos_detalle_tot
    WHERE cve_tab = par_Tabla AND control = par_Control AND referencia = pref;
END;
$$ LANGUAGE plpgsql;

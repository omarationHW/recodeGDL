...
BEGIN
  ...
  SELECT COALESCE(MAX(lote_envio),0)+1 INTO v_lote FROM public.norequeribles WHERE recaud = p_recaud;
  -- Actualizar registros
  UPDATE public.norequeribles SET lote_envio = v_lote, fecha_envio = CURRENT_DATE
...
DECLARE
  v_count integer := 0;
BEGIN
  UPDATE public.norequeribles SET user_baja = p_user, fecbaja = CURRENT_DATE
...
DECLARE
  v_id integer;
BEGIN
  INSERT INTO public.norequeribles (recaud, urbrus, cuenta, cvecuenta, feccap, capturista, observacion, fecbaja, tipo_bloq, fecha_envio, lote_envio)
...
DECLARE
  v_id integer;
BEGIN
  UPDATE public.norequeribles SET user_baja = p_usuario, fecbaja = p_fecha_desbloqueo, observacion = observacion || '\n' || p_motivo, tipo_bloq = 200

...
-- Archivo: 02_SP_TRAMITETRUNK_BUSQUE_EXACTO_all_procedures.sql
-- Generado: 2025-09-10
-- Total SPs: 5 (EXACTO)
-- ============================================
...
BEGIN
  ...
  FROM tramitetrunk.regprop a
  JOIN tramitetrunk.contrib b ON b.cvecont = a.cvecont
  JOIN tramitetrunk.convcta c ON c.cvecuenta = a.cvecuenta
  JOIN tramitetrunk.ubicacion d ON d.cveubic = g.cveubic
  JOIN tramitetrunk.catastro g ON g.cvecuenta = a.cvecuenta AND g.cveregprop = a.cveregprop
  LEFT JOIN tramitetrunk.c_calidpro h ON h.cvereg = a.cvereg
...
BEGIN
  ...
  FROM tramitetrunk.convcta a
  JOIN tramitetrunk.catastro b ON b.cvecuenta = a.cvecuenta
  JOIN tramitetrunk.regprop c ON c.cvecuenta = a.cvecuenta AND c.cveregprop = b.cveregprop AND c.encabeza = 'S'
  JOIN tramitetrunk.contrib d ON d.cvecont = c.cvecont
  JOIN tramitetrunk.ubicacion f ON f.cvecuenta = a.cvecuenta AND f.vigencia = 'V'
  LEFT JOIN tramitetrunk.c_calidpro h ON h.cvereg = c.cvereg

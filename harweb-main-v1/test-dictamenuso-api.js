// Test script para verificar la API de dictámenes uso de suelo
const fetch = require('node-fetch');

async function testDictamenUsoAPI() {
  console.log('=== PRUEBA DE API DICTÁMENES USO DE SUELO ===\n');

  try {
    const eRequest = {
      Operacion: 'sp_dictamenuso_list',
      Base: 'padron_licencias',
      Parametros: [
        { nombre: 'p_axo', valor: null },
        { nombre: 'p_folio', valor: null },
        { nombre: 'p_solicita', valor: null },
        { nombre: 'p_licencia', valor: null },
        { nombre: 'p_vigente', valor: null },
        { nombre: 'p_fecha_ini', valor: null },
        { nombre: 'p_fecha_fin', valor: null },
        { nombre: 'p_limite', valor: 5 },
        { nombre: 'p_offset', valor: 0 }
      ],
      Tenant: 'informix'
    };

    console.log('Request:', JSON.stringify({ eRequest }, null, 2));

    const response = await fetch('http://localhost:8080/api/generic', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ eRequest })
    });

    const data = await response.json();
    console.log('Response structure:', {
      success: data.success,
      hasData: !!data.data,
      dataLength: data.data?.length || 0
    });

    if (data.success && data.data) {
      console.log('✅ sp_dictamenuso_list funciona correctamente');
      console.log(`   Constancias encontradas: ${data.data.length}`);
      if (data.data.length > 0) {
        console.log('   Primera constancia:', data.data[0]);
        console.log('\n   Campos de la primera constancia:');
        Object.keys(data.data[0]).forEach(key => {
          console.log(`     ${key}: ${data.data[0][key]}`);
        });
      }
    } else {
      console.log('❌ sp_dictamenuso_list falló:', data.error || 'Sin datos');
    }

  } catch (error) {
    console.log('❌ Error en la prueba:', error.message);
  }
}

testDictamenUsoAPI();
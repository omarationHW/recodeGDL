...
if ($path === '/api/generic' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $host = $envVars['DB_HOST'] ?? '68.155.217.137';
        $port = $envVars['DB_PORT'] ?? 5432;
        $dbname = $envVars['DB_DATABASE'] ?? 'postgres';
        $username = $envVars['DB_USERNAME'] ?? 'postgres';
        $password = $envVars['DB_PASSWORD'] ?? 'R3c0d325';
        $schema = $envVars['DB_SCHEMA'] ?? 'pavimentacion';

        // Construir nombre del stored procedure
        $procedure = $operacion;
        if (!str_starts_with($procedure, 'sp_')) {
            $procedure = 'sp_' . $procedure;
        }
        ...
        try {
            ...
            $spFullName = $schema . '.sp_' . $operacion;
            ...
            ");
            $checkSP->execute([$schema, 'sp_' . $operacion]);
            $spInfo = $checkSP->fetch(PDO::FETCH_ASSOC);
            
            if (!$spInfo) {
                throw new Exception("Stored Procedure '{$spFullName}' no existe en el esquema '{$schema}'. Verifique el nombre del SP en el eRequest.Operacion");
            }

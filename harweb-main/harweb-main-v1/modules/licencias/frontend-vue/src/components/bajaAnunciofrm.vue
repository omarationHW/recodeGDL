          ...
          <div class="col-md-6">
            <table class="table table-borderless table-sm">
              <tbody>
                ...
                <tr>
                  <th class="text-muted">Propietario:</th>
                  <td>{{ anuncioData.propietarionvo || anuncioData.propietario }}</td>
                </tr>
                <tr>
            ...
            <table class="table table-borderless table-sm">
              <tbody>
                ...
                <tr>
                  ...
                  <td>{{ anuncioData.telefono_prop || 'No registrado' }}</td>
                </tr>
                <tr>
...
export default {
  ...
  setup() {
    ...
    const buscarAnuncio = async () => {
      if (!filtros.anuncio) {
        error.value = 'Ingrese el número de anuncio'
        return
      }
      ...
      try {
        const eRequest = {
          ...
          Parametros: [
            {
              nombre: 'p_anuncio',
              valor: parseInt(filtros.anuncio),
              tipo: 'integer'
            }
          ],
          Tenant: 'guadalajara'
        ...
        if (response.success && response.data.result && response.data.result.length > 0) {
          anuncioData.value = response.data.result[0]
          
          // Parsear adeudos si vienen como JSON string
          if (anuncioData.value.adeudos && typeof anuncioData.value.adeudos === 'string') {
            try {
              anuncioData.value.adeudos = JSON.parse(anuncioData.value.adeudos)
            } catch (e) {
              anuncioData.value.adeudos = []
            }
          }
        } else {
          error.value = 'No se encontró el anuncio especificado'
        }
      } catch (e) {
        console.error('Error al buscar anuncio:', e)
        error.value = 'Error al consultar el anuncio: ' + (e.message || 'Error desconocido')
      } finally {
        loading.value = false
      }
    }
    ...
    const procesarBaja = async () => {
      if (!formBaja.motivo.trim()) {
        error.value = 'Debe especificar un motivo para la baja'
        return
      }
      ...
      try {
        const eRequest = {
          ...
          Parametros: [
            ...
            {
              nombre: 'p_motivo',
              valor: formBaja.motivo,
              tipo: 'string'
            },
            {
              nombre: 'p_axo_baja',
              valor: formBaja.baja_error || formBaja.baja_tiempo ? null : parseInt(formBaja.axo_baja),
              tipo: 'integer'
            },
            {
              nombre: 'p_folio_baja',
              valor: formBaja.baja_error || formBaja.baja_tiempo ? null : parseInt(formBaja.folio_baja),
              tipo: 'integer'
            },
            {
              nombre: 'p_usuario',
              valor: 'sistema', // TODO: obtener usuario actual
              tipo: 'string'
            },
            {
              nombre: 'p_baja_error',
              valor: formBaja.baja_error,
              tipo: 'boolean'
            },
            {
              nombre: 'p_baja_tiempo',
              valor: formBaja.baja_tiempo,
              tipo: 'boolean'
            }
          ],
          Tenant: 'guadalajara'

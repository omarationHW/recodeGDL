...
BEGIN
  RETURN QUERY
    ...
    LEFT JOIN public.ta_12_passwords u ON e.id_usuario = u.id_usuario
...
DECLARE
  ...
  recargo NUMERIC;
BEGIN
  FOR r IN SELECT a.axo, a.periodo, a.importe, a.id_energia FROM public.ta_11_adeudo_energ a
           JOIN public.ta_11_energia e ON a.id_energia = e.id_energia
           WHERE e.id_local = p_id_local
...
BEGIN
  RETURN QUERY
    SELECT SUM(porcentaje_mes) FROM public.ta_12_recargos WHERE axo = p_axo AND mes >= p_mes;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  RETURN QUERY
    SELECT EXTRACT(DAY FROM fecha_limite)::SMALLINT FROM public.ta_12_diaslimite WHERE EXTRACT(MONTH FROM fecha_limite) = p_mes;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  RETURN QUERY
    ...
    FROM public.ta_11_pago_energia p
...
BEGIN
  RETURN QUERY
    ...
    LEFT JOIN public.ta_12_passwords u ON c.id_usuario = u.id_usuario

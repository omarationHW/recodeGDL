...
-- Formulario: RActualiza (EXACTO del archivo original)
-- Archivo: 17_SP_OTRASOBLIG_RACTUALIZA_EXACTO_all_procedures.sql
-- Generado: 2025-09-10
-- Total SPs: 3 (EXACTO)
-- ============================================
...
BEGIN
    ...
    LEFT JOIN otrasoblig.t34_adicionales B ON B.id_datos = A.id_34_datos
    LEFT JOIN otrasoblig.t34_conceptos C ON C.id_datos = A.id_34_datos AND C.tipo = 'N'
    LEFT JOIN otrasoblig.t34_conceptos D ON D.id_datos = A.id_34_datos AND D.tipo = 'L'
    LEFT JOIN otrasoblig.t34_conceptos E ON E.id_datos = A.id_34_datos AND E.tipo = 'O'
    JOIN otrasoblig.t34_status Z ON Z.id_34_stat = A.id_stat
    JOIN otrasoblig.t34_unidades F ON F.id_34_unidad = A.id_unidad
...
DECLARE
    ...
    v_old_descrip TEXT;
BEGIN
    SELECT concesionario, ubicacion, licencia, superficie, (SELECT descripcion FROM otrasoblig.t34_unidades WHERE id_34_unidad = otrasoblig.t34_datos.id_unidad)
    INTO v_old_concesionario, v_old_ubicacion, v_old_licencia, v_old_superficie, v_old_descrip
    FROM otrasoblig.t34_datos WHERE id_34_datos = id_34_datos;
    ...
    IF opc = 0 THEN
        IF v_old_concesionario = concesionario THEN
            ...
            RETURN;
        END IF;
        UPDATE otrasoblig.t34_datos SET concesionario = concesionario WHERE id_34_datos = id_34_datos;
    ...
    ELSIF opc = 1 THEN
        IF v_old_ubicacion = ubicacion THEN
            ...
            RETURN;
        END IF;
        UPDATE otrasoblig.t34_datos SET ubicacion = ubicacion WHERE id_34_datos = id_34_datos;
    ...
    ELSIF opc = 2 THEN
        IF v_old_licencia = licencia THEN
            ...
            RETURN;
        END IF;
        UPDATE otrasoblig.t34_datos SET licencia = licencia WHERE id_34_datos = id_34_datos;
    ...
    ELSIF opc = 3 THEN
        IF v_old_superficie = superficie THEN
            ...
            RETURN;
        END IF;
        UPDATE otrasoblig.t34_datos SET superficie = superficie WHERE id_34_datos = id_34_datos;
    ...
    ELSIF opc = 4 THEN
        IF v_old_descrip = descrip THEN
            ...
            RETURN;
        END IF;
        UPDATE otrasoblig.t34_datos SET id_unidad = (SELECT id_34_unidad FROM otrasoblig.t34_unidades WHERE descripcion = descrip LIMIT 1) WHERE id_34_datos = id_34_datos;
        RETURN QUERY SELECT 0, 'Tipo de local actualizado correctamente';
    ELSIF opc = 5 THEN
        -- Inicio de obligación (actualizar fecha_inicio)
        UPDATE otrasoblig.t34_datos SET fecha_inicio = make_date(aso_ini, mes_ini, 1) WHERE id_34_datos = id_34_datos;

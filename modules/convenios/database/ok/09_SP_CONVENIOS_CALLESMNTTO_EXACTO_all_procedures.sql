...
-- Formulario: CallesMntto (EXACTO del archivo original)
-- Archivo: 09_SP_CONVENIOS_CALLESMNTTO_EXACTO_all_procedures.sql
-- Generado: 2025-09-10
-- Total SPs: 7 (EXACTO)
-- ============================================
...
BEGIN
  RETURN QUERY SELECT colonia, descripcion FROM public.ta_17_colonias ORDER BY colonia;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  RETURN QUERY SELECT servicio, descripcion FROM public.ta_17_servicios ORDER BY servicio;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  RETURN QUERY SELECT cta_aplicacion, descripcion FROM public.cg_12_cuentas;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  INSERT INTO public.ta_17_calles (

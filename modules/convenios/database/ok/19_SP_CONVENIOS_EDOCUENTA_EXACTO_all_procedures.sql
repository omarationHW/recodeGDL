...
-- Formulario: EdoCuenta (EXACTO del archivo original)
-- Archivo: 19_SP_CONVENIOS_EDOCUENTA_EXACTO_all_procedures.sql
-- Generado: 2025-09-10
-- Total SPs: 4 (EXACTO)
-- ============================================
...
BEGIN
  ...
  SELECT
    ...
    COALESCE((SELECT SUM(importe_pago) FROM public.ta_17_conv_pagos WHERE id_conv_resto = b.id_conv_resto), 0) AS total_importe,
    COALESCE((SELECT SUM(COALESCE(importe_recargo,0)) FROM public.ta_17_conv_pagos WHERE id_conv_resto = b.id_conv_resto), 0) AS total_recargos
  FROM public.ta_17_con_reg_pred a
  JOIN public.ta_17_conv_d_resto b ON b.tipo = a.tipo AND b.id_conv_diver = a.id_conv_predio
  JOIN public.ta_17_adeudos_div c ON c.id_conv_resto = b.id_conv_resto AND c.clave_pago IS NULL
...
BEGIN
  ...
  JOIN public.ta_17_conv_d_resto b ON b.tipo = a.tipo AND b.id_conv_diver = a.id_conv_predio
  JOIN public.ta_17_adeudos_div c ON c.id_conv_resto = b.id_conv_resto AND c.clave_pago IS NULL
  LEFT JOIN public.ta_17_conv_pagos p ON p.id_conv_resto = b.id_conv_resto
  WHERE a.tipo = p_tipo AND a.subtipo = p_subtipo
...
BEGIN
  ...
  SELECT fecha_venc, importe INTO fecha1, importe FROM public.ta_17_adeudos_div WHERE id_conv_resto = p_id_conv_resto AND pago_parcial = p_pago_parcial;
  ...
  SELECT EXTRACT(DAY FROM fecha_venc) INTO dvenc FROM public.ta_17_adeudos_div WHERE id_conv_resto = p_id_conv_resto AND EXTRACT(YEAR FROM fecha_venc) = alo AND EXTRACT(MONTH FROM fecha_venc) = mes LIMIT 1;
  ...
  END IF;
  SELECT COALESCE(SUM(porcentaje_parcial),0) INTO porc FROM public.ta_13_recargosrcm WHERE (axo = alov AND mes >= mesv) OR (axo = alo AND mes <= CASE WHEN dia <= dvenc THEN mes-1 ELSE mes END) OR (axo > alov AND axo < alo);

-- Stored Procedure: rep_est_lic_report4
-- Tipo: Report
-- Descripción: Licencias reglamentadas dadas de alta en un rango de tiempo (clasificación C, D o ambas)
-- Generado para formulario: repEstadisticosLicfrm
-- Fecha: 2025-08-26 17:53:26

CREATE OR REPLACE FUNCTION rep_est_lic_report4(fecha1 DATE, fecha2 DATE, clasificacion TEXT)
RETURNS TABLE(
  id_giro INTEGER,
  descripcion TEXT,
  clasificacion TEXT,
...
BEGIN
  IF clasificacion = 'C' THEN
    filtro := 'c.clasificacion = ''C''';
  ELSIF clasificacion = 'D' THEN
    filtro := 'c.clasificacion = ''D''';
  ELSE
    filtro := '(c.clasificacion = ''C'' OR c.clasificacion = ''D'')';
  END IF;
  RETURN QUERY EXECUTE format('
    SELECT l.id_giro, c.descripcion, c.clasificacion,
    ...
    WHERE l.vigente = ''V'' AND l.fecha_otorgamiento >= $1 AND l.fecha_otorgamiento <= $2 AND %s
    GROUP BY l.id_giro, c.descripcion, c.clasificacion
    ORDER BY c.clasificacion, c.descripcion', filtro)
    USING fecha1, fecha2;
END;
$$ LANGUAGE plpgsql;
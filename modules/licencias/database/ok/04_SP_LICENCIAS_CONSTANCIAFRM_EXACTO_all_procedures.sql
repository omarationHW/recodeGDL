...
BEGIN
    SELECT constancia INTO v_folio FROM public.parametros ORDER BY id DESC LIMIT 1;
    v_folio := COALESCE(v_folio, 0) + 1;
    UPDATE public.parametros SET constancia = v_folio;
    INSERT INTO public.constancias(axo, folio, tipo, solicita, partidapago, observacion, domicilio, id_licencia, vigente, feccap, capturista)
    VALUES (v_axo, v_folio, p_tipo, p_solicita, p_partidapago, p_observacion, p_domicilio, p_id_licencia, 'V', CURRENT_DATE, p_capturista);
    RETURN QUERY SELECT * FROM public.constancias WHERE axo = v_axo AND folio = v_folio;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    RETURN QUERY SELECT * FROM public.constancias WHERE axo = p_axo AND folio = p_folio;
END;
$$ LANGUAGE plpgsql;
...
RETURNS TABLE(axo integer, folio integer, id_licencia integer, solicita varchar, partidapago varchar, observacion varchar, domicilio varchar, tipo smallint, vigente varchar, feccap date, capturista varchar) AS $$
BEGIN
    RETURN QUERY SELECT * FROM public.constancias ORDER BY axo DESC, folio DESC;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    RETURN QUERY SELECT * FROM public.constancias

...
BEGIN
    ...
    ELSE
        v_tipo_emision := 'M';
    END IF;
    IF EXISTS (SELECT 1 FROM public.ta_11_mercados WHERE oficina = p_oficina AND num_mercado_nvo = p_num_mercado_nvo) THEN
...
BEGIN
    RETURN QUERY SELECT id_zona, zona FROM public.ta_12_zonas ORDER BY id_zona;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    RETURN QUERY SELECT cta_aplicacion, descripcion FROM public.ta_12_cuentas ORDER BY cta_aplicacion;
END;
$$ LANGUAGE plpgsql;

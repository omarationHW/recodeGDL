...
DECLARE
  existe integer;
BEGIN
  SELECT COUNT(*) INTO existe FROM public.ta_11_categoria WHERE categoria = p_categoria;
  IF existe > 0 THEN
    RETURN QUERY SELECT false, 'La categoría ya existe';
    RETURN;
  END IF;
  INSERT INTO public.ta_11_categoria (categoria, descripcion) VALUES (p_categoria, UPPER(p_descripcion));
...
BEGIN
  ...
  IF existe = 0 THEN
    ...
    RETURN;
  END IF;
  UPDATE public.ta_11_categoria SET descripcion = UPPER(p_descripcion) WHERE categoria = p_categoria;

...
DECLARE
  new_id integer;
BEGIN
  SELECT COALESCE(MAX(categoria),0)+1 INTO new_id FROM public.ta_11_categoria;
  INSERT INTO public.ta_11_categoria (categoria, descripcion) VALUES (new_id, UPPER(_descripcion));
  RETURN QUERY SELECT categoria, descripcion FROM public.ta_11_categoria WHERE categoria = new_id;
END; $$;
...
LANGUAGE plpgsql AS $$
BEGIN
  UPDATE public.ta_11_categoria SET descripcion = UPPER(_descripcion) WHERE categoria = _categoria;
  RETURN QUERY SELECT categoria, descripcion FROM public.ta_11_categoria WHERE categoria = _categoria;
END; $$;
...
LANGUAGE plpgsql AS $$
BEGIN
  INSERT INTO public.ta_11_secciones (seccion, descripcion) VALUES (UPPER(_seccion), UPPER(_descripcion));
  RETURN QUERY SELECT seccion, descripcion FROM public.ta_11_secciones WHERE seccion = UPPER(_seccion);
END; $$;
...
LANGUAGE plpgsql AS $$
BEGIN
  UPDATE public.ta_11_secciones SET descripcion = UPPER(_descripcion) WHERE seccion = UPPER(_seccion);
  RETURN QUERY SELECT seccion, descripcion FROM public.ta_11_secciones WHERE seccion = UPPER(_seccion);
END; $$;
...
LANGUAGE plpgsql AS $$
BEGIN
  INSERT INTO public.ta_11_cve_cuota (clave_cuota, descripcion) VALUES (_clave, UPPER(_descripcion));
  RETURN QUERY SELECT clave_cuota, descripcion FROM public.ta_11_cve_cuota WHERE clave_cuota = _clave;
END; $$;
...
LANGUAGE plpgsql AS $$
BEGIN
  UPDATE public.ta_11_cve_cuota SET descripcion = UPPER(_descripcion) WHERE clave_cuota = _clave;
  RETURN QUERY SELECT clave_cuota, descripcion FROM public.ta_11_cve_cuota WHERE clave_cuota = _clave;
END; $$;
...
LANGUAGE plpgsql AS $$
BEGIN
  RETURN QUERY SELECT oficina, num_mercado_nvo, categoria, descripcion, cuenta_ingreso, cuenta_energia, id_zona, tipo_emision FROM public.ta_11_mercados ORDER BY oficina, num_mercado_nvo;
END; $$;
...
LANGUAGE plpgsql AS $$
BEGIN
  INSERT INTO public.ta_11_mercados (oficina, num_mercado_nvo, categoria, descripcion, cuenta_ingreso, cuenta_energia, id_zona, tipo_emision)
  VALUES (_oficina, _num_mercado_nvo, _categoria, UPPER(_descripcion), _cuenta_ingreso, _cuenta_energia, _zona, _tipo_emision);
  RETURN QUERY SELECT oficina, num_mercado_nvo, categoria, descripcion FROM public.ta_11_mercados WHERE oficina = _oficina AND num_mercado_nvo = _num_mercado_nvo;
END; $$;
...
LANGUAGE plpgsql AS $$
BEGIN
  UPDATE public.ta_11_mercados SET categoria=_categoria, descripcion=UPPER(_descripcion), cuenta_ingreso=_cuenta_ingreso, cuenta_energia=_cuenta_energia, id_zona=_zona, tipo_emision=_tipo_emision

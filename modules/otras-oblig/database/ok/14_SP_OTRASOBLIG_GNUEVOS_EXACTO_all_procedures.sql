...
-- Formulario: GNuevos (EXACTO del archivo original)
-- Archivo: 14_SP_OTRASOBLIG_GNUEVOS_EXACTO_all_procedures.sql
-- Generado: 2025-09-10
-- Total SPs: 1 (EXACTO)
-- ============================================

-- SP 1/1: sp_gnuevos_alta
-- Tipo: CRUD
...
BEGIN
    ...
    SELECT count(*) INTO v_exists FROM otrasoblig.t34_datos WHERE control = par_control AND cve_tab = par_tabla;
    IF v_exists > 0 THEN
        RETURN QUERY SELECT 0, 'Ya existe un registro con ese control';
        RETURN;
    END IF;
    -- Insertar en otrasoblig.t34_datos
    ...
    -- Insertar en otrasoblig.t34_conceptos si aplica (ejemplo)
    -- INSERT INTO otrasoblig.t34_conceptos (id_datos, tipo, concepto) VALUES (v_id_34_datos, 'C', par_NomCom);
    -- ...

...
BEGIN
  RETURN QUERY
    ...
    LEFT JOIN public.norequeriblelic n ON n.id_licencia = l.id_licencia AND n.fecbaja IS NULL
...
BEGIN
  SELECT id_licencia INTO v_id_licencia FROM public.licencias WHERE licencia = p_licencia;
  IF v_id_licencia IS NULL THEN
    RETURN QUERY SELECT 'Licencia no encontrada';
    RETURN;
  END IF;
  -- Verifica si ya está bloqueada
  IF EXISTS (SELECT 1 FROM public.norequeriblelic WHERE id_licencia = v_id_licencia AND fecbaja IS NULL) THEN
    RETURN QUERY SELECT 'La licencia ya está bloqueada';
    RETURN;
  END IF;
  INSERT INTO public.norequeriblelic (id_licencia, feccap, capturista, observacion, tipo_bloq)
...
BEGIN
  IF p_status = 'blocked' THEN
    RETURN QUERY
      ...
      FROM public.norequeriblelic n
      JOIN public.licencias l ON l.id_licencia = n.id_licencia

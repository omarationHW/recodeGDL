...
BEGIN
    IF p_user_name = 'proceso' AND p_fecha IS NOT NULL THEN
        ...
        FROM public.pagos p
        LEFT JOIN public.gastmult g ON g.cvepago = p.cvepago
        LEFT JOIN public.imprectp tp ON tp.cvepago = p.cvepago
...
CREATE OR REPLACE FUNCTION verificar_no_adeudo(
    p_cvecuenta INTEGER,
    p_cvepago INTEGER
) RETURNS SETOF public.noadeudo AS $$
BEGIN
    RETURN QUERY
    SELECT a.*
    FROM public.noadeudo a
    JOIN public.pagos p ON a.cvecuenta = p.cvecuenta
    JOIN public.imprecpre i ON i.cvepago = p.cvepago
    WHERE p.cvecuenta = p_cvecuenta
      ...
      AND a.cvepago NOT IN (
        SELECT cvepago FROM public.pagos
...
BEGIN
    ...
    IF p_cveconcepto = 6 OR p_cveconcepto = 7 THEN
        UPDATE public.reqmultas SET vigencia = 'V', cvepago = NULL WHERE cvepago = p_cvepago;
    END IF;
    ...
    INSERT INTO public.pagoscan (cvepago, autorizo, fechacan)
    VALUES (p_cvepago, p_user_name, p_fecha_cancelacion);
    -- Marcar el pago como cancelado (si aplica en la tabla pagos)
    UPDATE public.pagos SET cvecanc = (SELECT currval(pg_get_serial_sequence('public.pagoscan','cvecanc'))) WHERE cvepago = p_cvepago;

...
BEGIN
    ...
    FROM public.convcta WHERE cvecatnva = p_clave;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    FROM public.detsaldos WHERE cvecuenta = p_cvecuenta ORDER BY axosal, bimsal;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    FROM public.descpred WHERE cvecuenta = p_cvecuenta ORDER BY fecalta DESC;
END;
$$ LANGUAGE plpgsql;
...
RETURNS VOID AS $$
BEGIN
    UPDATE public.descpred SET status = 'C', fecbaja = NOW(), captbaja = p_usuario WHERE id = p_id;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    RETURN QUERY SELECT usuario, nombres, cvedepto FROM public.usuarios WHERE fecbaj IS NULL;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    RETURN QUERY SELECT cvedescuento, descripcion FROM public.c_descpred ORDER BY descripcion;
END;
$$ LANGUAGE plpgsql;

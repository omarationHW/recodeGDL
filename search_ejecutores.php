...
try {
    ...
    // 1. Buscar tablas con patrones relacionados a ejecutores
    echo "=== BUSCANDO TABLAS RELACIONADAS CON EJECUTORES ===\n\n";
    ...
    $keyFields = ['nombre', 'cargo', 'area', 'telefono', 'email', 'status'];

    echo "Buscando tablas que contengan campos relacionados con ejecutores\n\n";
    ...
    $stmt = $pdo->query("
        ...
        AND (
            LOWER(c.column_name) IN ('id_ejecutor', 'cveejecutor', 'cveejecut', 'numero_empleado', 'nombre_completo', 'nombre', 'cargo', 'area', 'telefono', 'extension', 'email', 'fecha_ingreso', 'status', 'observaciones')
            OR c.column_name ILIKE '%ejecut%'

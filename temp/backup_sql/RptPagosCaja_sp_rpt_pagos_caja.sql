...
BEGIN
    ...
    SELECT
        ...
        MIN(p.fecha_pago) as fecha_inicio,
        MAX(p.fecha_pago) as fecha_fin
    FROM
        padron_licencias.public.ta_11_pagos_local p
    INNER JOIN
        padron_licencias.comun.ta_11_locales l ON l.id_local = p.id_local
    INNER JOIN
        padron_licencias.comun.ta_11_mercados m ON m.num_mercado = l.num_mercado AND m.oficina = l.oficina
    WHERE
...
-- Comentario del SP
COMMENT ON FUNCTION public.sp_rpt_pagos_caja(INTEGER, DATE, DATE, VARCHAR) IS

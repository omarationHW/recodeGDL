#!/usr/bin/env python3
"""
Script para convertir DIAGRAMAS_CLASES_SUBSISTEMAS.md a Word (.docx)
    ...
    for line in lines:
        ...
        if line.startswith('```'):
            if not in_code_block:
                ...
                # Si es SQL, JSON u otro código, agregar como código
                elif code_lines:
                    p = doc.add_paragraph()
                    code_text = '\n'.join(code_lines)
...
if __name__ == '__main__':
    ...
    md_file = os.path.join(project_root, 'DIAGRAMAS_CLASES_SUBSISTEMAS.md')
    output_file = os.path.join(project_root, 'DIAGRAMAS_CLASES_SUBSISTEMAS.docx')

...
BEGIN
    ...
    adeudos AS (
        ...
        FROM comun.ta_11_locales l
        JOIN db_ingresos.ta_11_adeudo_local a ON a.id_local = l.id_local
        WHERE l.oficina = p_oficina
          AND l.vigencia = 'A'
          AND a.axo = p_axo
          AND a.periodo <= p_mes
    ...
    -- Capturas (locales activos con su cuota)
    capturas AS (
        SELECT
            l.num_mercado,
            COUNT(DISTINCT l.id_local) as locales,
...
foreach ($rows as $row) {
    echo "Mercado {$row['num_mercado_nvo']}: " . substr($row['descripcion'], 0, 20) . "\n";
    echo "  Pagados: {$row['localpag']} locales, \${$row['pagospag']}\n";
    echo "  Capturados: {$row['localcap']} locales, \${$row['pagoscap']}\n";

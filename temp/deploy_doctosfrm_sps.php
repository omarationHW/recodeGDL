<?php
/**
 * Deploy Script - Stored Procedures para doctosfrm.vue
 *
 * Componente: doctosfrm.vue
 ...
 *
 * SPs a desplegar:
 * 1. sp_doctos_list - Lista todos los tipos de documentos
 * 2. sp_doctos_create - Crea un nuevo tipo de documento
 * 3. sp_doctos_update - Actualiza un tipo de documento
 * 4. sp_doctos_delete - Elimina un tipo de documento
 */
...
echo "\n";
echo "╔═══════════════════════════════════════════════════════════════════╗\n";
echo "║  Deploy Stored Procedures - doctosfrm.vue                        ║\n";
echo "║  Módulo: Padrón de Licencias - Catálogo de Documentos           ║\n";
echo "╚═══════════════════════════════════════════════════════════════════╝\n";
echo "\n";
...
foreach ($statements as $index => $statement) {
    ...
    if (stripos($statement, 'DROP FUNCTION') !== false) {
        ...
        preg_match('/CREATE OR REPLACE FUNCTION\s+(?:public\.)?([a-z_]+)\(/i', $statement, $matches);
        $commandName = 'CREATE ' . ($matches[1] ?? 'FUNCTION');
    } elseif (stripos($statement, 'COMMENT ON FUNCTION') !== false) {
        preg_match('/COMMENT ON FUNCTION\s+(?:public\.)?([a-z_]+)\(/i', $statement, $matches);
        $commandName = 'COMMENT ' . ($matches[1] ?? 'FUNCTION');
    } elseif (stripos($statement, 'GRANT EXECUTE') !== false) {
        preg_match('/GRANT EXECUTE ON FUNCTION\s+(?:public\.)?([a-z_]+)\(/i', $statement, $matches);
...
if ($errorCount === 0) {
    ...
    echo "\n";
    echo "📋 Stored Procedures creados (esquema: public):\n";
    echo "   • sp_doctos_list\n";
    echo "   • sp_doctos_create\n";
    echo "   • sp_doctos_update\n";
    echo "   • sp_doctos_delete\n";
    echo "\n";
    ...
    echo "   SELECT * FROM public.sp_doctos_list();\n";
    echo "\n";

    echo "✅ doctosfrm.vue está listo para funcionar!\n";
} else {

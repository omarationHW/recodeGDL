 ...
 *
 * Este SP actualiza la fecha de práctica (fecent) de folios en reqpredial
...
try {
    ...
    // Desplegar SP recaudadora_actualiza_fechas
...
--   - p_ejecutor: Ejecutor (opcional, para futuras extensiones)
        ...
        BEGIN
            ...
            FOR v_folio IN SELECT * FROM jsonb_array_elements(v_folios) LOOP
                BEGIN
                    ...
                    -- Buscar el cvereq correspondiente
                    ...
                    IF v_cvereq IS NOT NULL THEN
                        -- Actualizar la fecha de práctica
                        UPDATE reqpredial
                    ...
                    ELSE
                        v_errores := v_errores || jsonb_build_object(
                            'error', 'Folio no encontrado en reqpredial',
                            'folio', v_folio_num,
                            'clave_cuenta', v_cvecuenta,
                            'anio_folio', v_anio
                        );
...
IS 'Actualiza fecha de práctica de folios en reqpredial (individual o lote)';
";

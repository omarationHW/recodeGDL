<?php
// Desplegar y probar SP: recaudadora_repmultampalfrm
$host = '192.168.6.146';
$port = '5432';
...
try {
    ...
    echo "=== DESPLEGANDO SP: recaudadora_repmultampalfrm ===\n\n";

    $sql = file_get_contents(__DIR__ . '/recaudadora_repmultampalfrm.sql');
    $pdo->exec($sql);
    ...
    $stmt1 = $pdo->prepare("SELECT * FROM public.recaudadora_repmultampalfrm(?)");
    $stmt1->execute([2025]);
    ...
    $stmt2 = $pdo->prepare("SELECT * FROM public.recaudadora_repmultampalfrm(?)");
    $stmt2->execute([2024]);
    ...
    $stmt3 = $pdo->prepare("SELECT * FROM public.recaudadora_repmultampalfrm(?)");
    $stmt3->execute([2023]);

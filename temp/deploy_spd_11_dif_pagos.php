...
try {
    ...
    $sqlFile = __DIR__ . '/../RefactorX/Base/mercados/database/database/PagosDifIngresos_spd_11_dif_pagos.sql';
    $sql = file_get_contents($sqlFile);

    echo "Desplegando spd_11_dif_pagos...\n";
    $pdo->exec($sql);
    ...
    try {
        $stmt = $pdo->prepare("SELECT * FROM spd_11_dif_pagos(?, ?, ?)");
        ...
        if (count($results) > 0) {
            ...
            echo "  (No hay pagos con datos diferentes en ingresos)\n";
        }

...
echo "🔍 Buscando tablas relacionadas con 'sin ligar'...\n\n";

try {
    // Buscar tablas con 'ligar' o 'liga' en el nombre
    $tables = DB::connection('pgsql')->select("
        ...
        AND (
            tablename ILIKE '%ligar%'
            OR tablename ILIKE '%liga%'
            OR tablename ILIKE '%vincula%'
            OR tablename ILIKE '%link%'
            OR tablename ILIKE '%sin%liga%'
        )
    ...
    if (count($tables) > 0) {
        ...
        echo "❌ No se encontraron tablas con 'ligar' en el nombre\n\n";

        // Buscar tablas relacionadas con pagos o cuentas sin vincular
        echo "🔍 Buscando tablas de pagos sin vincular...\n\n";

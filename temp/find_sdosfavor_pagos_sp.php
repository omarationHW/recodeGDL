...
echo "🔍 Buscando SP RECAUDADORA_SDOSFAVOR_PAGOS...\n\n";
...
try {
    ...
    $sp = DB::connection('pgsql')->select("
        ...
        WHERE p.proname ILIKE 'recaudadora_sdosfavor_pagos'
    ");
    ...
    if (count($sp) > 0) {
        ...
        try {
            ...
            if (count($examples) > 0) {
                ...
                foreach ($examples as $i => $ex) {
                    ...
                    if ($cuenta) {
                        ...
                        foreach ($data as $key => $value) {
                            if ($c >= 5) break;
                            if (stripos($key, 'cuenta') === false && $value !== null) {
                                $val = $value;
            ...
            // Intentar con cuentas conocidas del módulo anterior
            echo "Probando con cuentas conocidas de SdosFavor_CtrlExp...\n\n";

            $testCuentas = ['295685', '142963', '103753'];
                ...
                try {
                    ...
                    if (count($result) > 0) {
                        ...
                        foreach ($data as $key => $value) {
                            if ($c >= 5) break;
                            $val = $value ?? 'NULL';
                            if (is_numeric($val) && $key !== 'cvecuenta' && $key !== 'folio') {
        ...
        echo "❌ SP RECAUDADORA_SDOSFAVOR_PAGOS no encontrado\n\n";

        // Buscar tablas relacionadas
        echo "🔎 Buscando tablas relacionadas con pagos de saldos a favor...\n\n";
        ...
        $tables = DB::connection('pgsql')->select("
            ...
            AND (
                tablename ILIKE '%sdo%favor%pago%'
                OR tablename ILIKE '%pago%favor%'
                OR tablename ILIKE '%sdos%pago%'
            )

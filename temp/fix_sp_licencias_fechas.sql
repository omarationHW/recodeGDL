-- =====================================================
-- FIX: Manejo de strings vacíos en fechas
-- =====================================================
...
BEGIN
    ...
    -- Convertir strings vacíos a NULL y luego a date
    v_fecha_desde := CASE
        WHEN p_fecha_desde IS NULL OR TRIM(p_fecha_desde) = '' THEN NULL
        ELSE p_fecha_desde::date
    END;

    v_fecha_hasta := CASE
        WHEN p_fecha_hasta IS NULL OR TRIM(p_fecha_hasta) = '' THEN NULL
        ELSE p_fecha_hasta::date
    END;
    ...
    WHERE l.licencia > 0
        AND (p_giro IS NULL OR p_giro = '' OR g.descripcion ILIKE '%' || p_giro || '%')
        AND (p_zona IS NULL OR p_zona = '' OR CAST(l.zona AS TEXT) = p_zona)
        ...
        AND (v_fecha_desde IS NULL OR l.fecha_otorgamiento >= v_fecha_desde)
        AND (v_fecha_hasta IS NULL OR l.fecha_otorgamiento <= v_fecha_hasta)
        AND (p_propietario IS NULL OR p_propietario = '' OR l.propietario ILIKE '%' || p_propietario || '%');
...
'SP Licencias Vigentes con manejo robusto de strings vacíos';

...
echo "=== Corrigiendo SPs para ConsPagosEnergia ===\n\n";

// Verificar columnas de ta_11_pago_energia
echo "Verificando columnas de db_ingresos.ta_11_pago_energia...\n";
$stmt = $pdo->query("
    SELECT column_name, data_type
    FROM information_schema.columns
    WHERE table_schema = 'db_ingresos' AND table_name = 'ta_11_pago_energia'
...
// SP 1: Buscar pagos energia por local (corregido)
echo "\n1. Corrigiendo sp_cons_pagos_energia_por_local...\n";
...
BEGIN
    ...
    JOIN db_ingresos.ta_11_pago_energia p ON p.id_local = l.id_local
...
// SP 2: Buscar pagos energia por fecha (corregido)
...
// SP 3: Obtener cajas por oficina (energia) - corregido
echo "3. Corrigiendo sp_get_cajas_energia...\n";
$pdo->exec("DROP FUNCTION IF EXISTS public.sp_get_cajas_energia(INTEGER)");
$pdo->exec("
CREATE OR REPLACE FUNCTION public.sp_get_cajas_energia(p_oficina INTEGER)
RETURNS TABLE(caja VARCHAR) AS \$\$
BEGIN
    RETURN QUERY
    SELECT DISTINCT p.caja_pago::VARCHAR as caja
    FROM db_ingresos.ta_11_pago_energia p
    WHERE p.oficina_pago = p_oficina
    ORDER BY caja;
END;
\$\$ LANGUAGE plpgsql;
");
...
echo "\nTest sp_get_cajas_energia (oficina 1):\n";
try {
    $stmt = $pdo->query("SELECT * FROM sp_get_cajas_energia(1) LIMIT 5");
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    echo "Cajas encontradas: " . count($rows) . "\n";
    foreach ($rows as $row) {
        echo "  Caja: {$row['caja']}\n";
    }
} catch (Exception $e) {

...
-- Descripción: Consulta de registros de multas
-- Parámetros:
...
DROP FUNCTION IF EXISTS public.recaudadora_reg(TEXT);

CREATE OR REPLACE FUNCTION public.recaudadora_reg(
    p_registro TEXT
)
RETURNS TABLE (
    id_multa INTEGER,
...
BEGIN
    ...
    v_limite := COALESCE((v_json->>'limite')::INTEGER, 10);
...
COMMENT ON FUNCTION public.recaudadora_reg(TEXT) IS 'Consulta de registros de multas con múltiples filtros';

<?php
// Verificar tablas y SPs de observaciones

try {
    $db = new PDO('pgsql:host=192.168.6.146;port=5432;dbname=padron_licencias', 'refact', 'FF)-BQk2');
    $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

    echo "=== VERIFICACIÓN DE OBSERVACIONES ===\n\n";

    // 1. Buscar tablas relacionadas con observaciones
    echo "📊 Buscando tablas relacionadas con 'observacion'...\n";
    echo str_repeat("-", 80) . "\n";
    ...
    $stmt = $db->query("
        ...
        FROM pg_tables
        WHERE (tablename LIKE '%observa%' OR tablename LIKE '%obs%')
    ...
    if (count($tables) > 0) {
        ...
        echo "❌ NO se encontraron tablas con 'observacion'\n";
    }
    echo str_repeat("-", 80) . "\n\n";

    // 2. Buscar SPs relacionados con observaciones
    foreach (['public', 'comun'] as $schema) {
        echo "🔧 SPs con 'observacion' en esquema '$schema':\n";
        echo str_repeat("-", 80) . "\n";
        ...
        $stmt = $db->query("
            ...
            WHERE n.nspname = '$schema'
              AND (p.proname LIKE '%observa%' OR p.proname LIKE '%obs%')
            ORDER BY p.proname
        ");

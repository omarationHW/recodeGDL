-- Actualización del Stored Procedure para firma electrónica
-- Usa la tabla firmas_electronicas del esquema public
...
BEGIN
    ...
    -- Verificar si el folio ya tiene firma (ahora busca en public.firmas_electronicas)
    ...
    -- Generar hash con md5 (en producción usar algoritmo más robusto como SHA-256)
    v_hash := md5(v_folio || v_usuario || NOW()::TEXT || COALESCE(v_datos_firma, ''));
    v_fecha := NOW();

    -- Insertar la firma (ahora en public.firmas_electronicas)
    BEGIN
...
-- Este SP recibe un JSON con los datos de una firma electrónica y los registra en la tabla public.firmas_electronicas
-- Campos del JSON esperados:
--   - folio: Folio único del documento a firmar (obligatorio)
--   - usuario: Usuario que realiza la firma (obligatorio)
--   - tipo: Tipo de firma o documento (ej: "REQUERIMIENTO", "NOTIFICACION", etc.)
--   - datos_firma: Datos de la firma electrónica (puede ser base64, certificado, etc.)
--   - ip_address: Dirección IP desde donde se firma
--   - dispositivo: Información del dispositivo usado
--   - observaciones: Observaciones adicionales
--
-- Retorna:
--   - success: true/false
--   - message: Mensaje de éxito o error
--   - folio: Folio del documento firmado
--   - fecha_firma: Fecha y hora de la firma
--   - usuario: Usuario que firmó

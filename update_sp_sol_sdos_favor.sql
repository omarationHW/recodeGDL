-- =====================================================
-- Stored Procedure: recaudadora_sol_sdos_favor
-- Base de Datos: multas_reglamentos
-- Esquema: publico
-- Descripción: Solicitudes de saldos a favor
--              Lista y busca solicitudes de saldos a favor
-- =====================================================

-- Eliminar función si existe
DROP FUNCTION IF EXISTS publico.recaudadora_sol_sdos_favor(VARCHAR);

-- Crear función
CREATE OR REPLACE FUNCTION publico.recaudadora_sol_sdos_favor(
...
COMMENT ON FUNCTION publico.recaudadora_sol_sdos_favor(VARCHAR) IS
'Consulta de solicitudes de saldos a favor.
Busca en la tabla public.solic_sdosfavor por múltiples criterios.
Parámetros:
  - p_cuenta: VARCHAR (opcional) - Busca por:
...
Retorna tabla con información completa de la solicitud incluyendo:
  ...
  - Datos de la cuenta
  - Domicilio completo (domicilio, exterior, interior, colonia, sector, código)
  - Contacto (teléfono, solicitante)
  - Estado (status)
  - Observaciones
  - Fechas (captura, término)
  - Datos de control (inconf, peticionario, documentos)
Límite: 100 registros';

-- Otorgar permisos
GRANT EXECUTE ON FUNCTION publico.recaudadora_sol_sdos_favor(VARCHAR) TO refact;
GRANT EXECUTE ON FUNCTION publico.recaudadora_sol_sdos_favor(VARCHAR) TO PUBLIC;
